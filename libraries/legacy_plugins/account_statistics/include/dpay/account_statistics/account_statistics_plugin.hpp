...
// time.
//
#ifndef DPAY_ACCOUNT_STATISTICS_SPACE_ID
#define DPAY_ACCOUNT_STATISTICS_SPACE_ID 10
#endif

#ifndef DPAY_ACCOUNT_STATISTICS_PLUGIN_NAME
#define DPAY_ACCOUNT_STATISTICS_PLUGIN_NAME "account_stats"
#endif
...
{
   account_stats_bucket_object_type    = ( DPAY_ACCOUNT_STATISTICS_SPACE_ID << 8 ),
   account_activity_bucket_object_type = ( DPAY_ACCOUNT_STATISTICS_SPACE_ID << 8 ) + 1
};
...
struct account_stats_bucket_object : public object< account_stats_bucket_object_type, account_stats_bucket_object >
{
   ...
   share_type           author_rewards_bbd = 0;                   ///< BBD paid for author rewards
   share_type           author_rewards_vests = 0;                 ///< VESTS paid for author rewards
   share_type           author_rewards_total_dpay_value = 0;     ///< BEX Value of author rewards
   share_type           author_rewards_payout_bbd_value = 0;      ///< BBD Value of author rewards at time of payout
   ...
   share_type           curation_rewards_dpay_value = 0;         ///< BEX Value of curation rewards
   share_type           curation_rewards_payout_bbd_value = 0;    ///< BBD Value of curation rewards at time of payout
   uint32_t             liquidity_reward_payouts = 0;             ///< Number of liquidity reward payouts
   share_type           liquidity_rewards = 0;                    ///< Amount of BEX paid as liquidity rewards
   ...
   share_type           dpay_sent = 0;                           ///< BEX sent from this account
   share_type           dpay_received = 0;                       ///< BEX received by this account
   share_type           bbd_sent = 0;                             ///< BBD sent from this account
   share_type           bbd_received = 0;                         ///< BBD received by this account
   uint32_t             bbd_interest_payments = 0;                ///< Number of times interest was paid to BBD
   share_type           bbd_paid_as_interest = 0;                 ///< Amount of BBD paid as interest
   ...
   share_type           dpay_vested = 0;                         ///< BEX vested by the account
   ...
   share_type           dpay_received_from_withdrawls = 0;       ///< BEX received from this account's vesting withdrawals
   share_type           dpay_received_from_routes = 0;           ///< BEX received from another account's vesting withdrawals
   share_type           vests_received_from_routes = 0;           ///< VESTS received from another account's vesting withdrawals
   uint32_t             bbd_conversion_requests_created = 0;      ///< BBD conversion requests created
   share_type           bbd_to_be_converted = 0;                  ///< Amount of BBD to be converted
   uint32_t             bbd_conversion_requests_filled = 0;       ///< BBD conversion requests filled
   share_type           dpay_converted = 0;                      ///< Amount of BEX that was converted
   ...
   share_type           limit_order_dpay_paid = 0;               ///< BEX paid by limit orders
   share_type           limit_order_dpay_received = 0;           ///< BEX received from limit orders
   share_type           limit_order_bbd_paid = 0;                 ///< BBD paid by limit orders
   share_type           limit_order_bbd_received = 0;             ///< BBD received by limit orders
...
class account_statistics_plugin : public dpay::app::plugin
{
   public:
      account_statistics_plugin( application* app );
      virtual ~account_statistics_plugin();

      virtual std::string plugin_name()const override { return DPAY_ACCOUNT_STATISTICS_PLUGIN_NAME; }
...
FC_REFLECT( dpay::account_statistics::account_stats_bucket_object,
   (id)
   (open)
   (seconds)
   (name)
   ...
   (author_rewards_total_dpay_value)
   (author_rewards_payout_bbd_value)
   (curation_reward_payouts)
   (curation_rewards_vests)
   (curation_rewards_dpay_value)
   (curation_rewards_payout_bbd_value)
   ...
   (bbd_sent)
   (bbd_received)
   (bbd_interest_payments)
   (bbd_paid_as_interest)
   ...
   (vests_withdrawn)
   (dpay_received_from_withdrawls)
   (dpay_received_from_routes)
   (vests_received_from_routes)
   (bbd_conversion_requests_created)
   (bbd_to_be_converted)
   (bbd_conversion_requests_filled)
   (dpay_converted)
   (limit_orders_created)
   (limit_orders_filled)
   (limit_orders_cancelled)
   (limit_order_dpay_paid)
   (limit_order_dpay_received)
   (limit_order_bbd_paid)
   (limit_order_bbd_received)
   (total_pow)
   (estimated_hashpower)
)
//SET_INDEX_TYPE( dpay::account_statistics::account_stats_bucket_object,)

FC_REFLECT(
   dpay::account_statistics::account_activity_bucket_object,
   (id)
   (open)
   (seconds)
