...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   {
      FC_ASSERT( o.url.size() <= DPAY_MAX_WITNESS_URL_LENGTH, "URL is too long" );
   }
   else if( o.url.size() > DPAY_MAX_WITNESS_URL_LENGTH )
   {
   ...
   {
      FC_ASSERT( o.props.account_creation_fee.symbol == DPAY_SYMBOL );
   }
   else if( o.props.account_creation_fee.symbol != DPAY_SYMBOL )
   {
      // after HF, above check can be moved to validate() if reindex doesn't show this warning
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( _db.has_hardfork( DPAY_HARDFORK_0_19__987) )
   {
   ...
   const auto& new_account = _db.create< account_object >( [&]( account_object& acc )
   {
      ...
      if( !_db.has_hardfork( DPAY_HARDFORK_0_11__169 ) )
...
{
   FC_ASSERT( _db.has_hardfork( DPAY_HARDFORK_0_17__818 ), "Account creation with delegation is not enabled until hardfork 17" );
...
{
   if( _db.has_hardfork( DPAY_HARDFORK_0_10 ) )
   {
...
{ try {
   ...
   if( ( _db.has_hardfork( DPAY_HARDFORK_0_17__926 ) ) && o.json_metadata.size() )
   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      if( _db.has_hardfork( DPAY_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == DPAY_ROOT_POST_PARENT )
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      {
         const auto& from_account = _db.get_account( o.from );
         _db.adjust_balance( from_account, escrow.dpay_balance );
         _db.adjust_balance( from_account, escrow.bbd_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      _db.adjust_balance( receiver_account, o.dpay_amount );
      _db.adjust_balance( receiver_account, o.bbd_amount );
...
void transfer_to_vesting_evaluator::do_apply( const transfer_to_vesting_operation& o )
{
   ...
   FC_ASSERT( _db.get_balance( from_account, DPAY_SYMBOL) >= o.amount, "Account does not have sufficient BEX for transfer." );
   _db.adjust_balance( from_account, -o.amount );
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   if( !account.mined && _db.has_hardfork( DPAY_HARDFORK_0_1 ) )
   {
      const auto& props = _db.get_dynamic_global_properties();
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
...
{ try {
   ...
   int64_t regenerated_power = (DPAY_100_PERCENT * elapsed_seconds) / DPAY_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(DPAY_100_PERCENT) );
   FC_ASSERT( current_power > 0, "Account currently does not have voting power." );

   int64_t  abs_weight    = abs(o.weight);
   int64_t  used_power    = (current_power * abs_weight) / DPAY_100_PERCENT;

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // used_power = (current_power * abs_weight / DPAY_100_PERCENT) * (reserve / max_vote_denom)
   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * DPAY_VOTE_REGENERATION_SECONDS / (60*60*24);
   ...
   int64_t abs_rshares    = ((uint128_t(voter.effective_vesting_shares().amount.value) * used_power) / (DPAY_100_PERCENT)).to_uint64();
...
void pow_apply( database& db, Operation o )
{
   ...
   if(itr == accounts_by_name.end())
   {
      db.create< account_object >( [&]( account_object& acc )
      {
         ...
         if( !db.has_hardfork( DPAY_HARDFORK_0_11__169 ) )
            acc.recovery_account = "dpay";
         else
...
{
   if( _db.has_hardfork( DPAY_HARDFORK_0_14__307 ) ) FC_ASSERT( false, "Challenge authority operation is currently disabled." );
   ...
   if( o.require_owner )
   {
      ...
      FC_ASSERT( challenger.balance >= DPAY_OWNER_CHALLENGE_FEE );
      FC_ASSERT( !challenged.owner_challenged );
      FC_ASSERT( _db.head_block_time() - challenged.last_owner_proved > DPAY_OWNER_CHALLENGE_COOLDOWN );

      _db.adjust_balance( challenger, - DPAY_OWNER_CHALLENGE_FEE );
      _db.create_vesting( _db.get_account( o.challenged ), DPAY_OWNER_CHALLENGE_FEE );
  ...
  {
      FC_ASSERT( challenger.balance >= DPAY_ACTIVE_CHALLENGE_FEE, "Account does not have sufficient funds to pay challenge fee." );
      FC_ASSERT( !( challenged.owner_challenged || challenged.active_challenged ), "Account is already challenged." );
      FC_ASSERT( _db.head_block_time() - challenged.last_active_proved > DPAY_ACTIVE_CHALLENGE_COOLDOWN, "Account cannot be challenged because it was recently challenged." );

      _db.adjust_balance( challenger, - DPAY_ACTIVE_CHALLENGE_FEE );
      _db.create_vesting( _db.get_account( o.challenged ), DPAY_ACTIVE_CHALLENGE_FEE );
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   ...
   auto min_delegation = asset( wso.median_props.account_creation_fee.amount * 10, DPAY_SYMBOL ) * gpo.get_vesting_share_price();
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      _db.create< vesting_delegation_expiration_object >( [&]( vesting_delegation_expiration_object& obj )
      {
         ...
         obj.expiration = std::max( _db.head_block_time() + DPAY_CASHOUT_WINDOW_SECONDS, delegation->min_delegation_time );
      });
