...
{
   FC_ASSERT( _db.has_hardfork( DPAY_SMT_HARDFORK ), "SMT functionality not enabled until hardfork ${hf}", ("hf", DPAY_SMT_HARDFORK) );
   ...
   FC_ASSERT( dgpo.smt_creation_fee.symbol == DPAY_SYMBOL || dgpo.smt_creation_fee.symbol == BBD_SYMBOL,
      "Unexpected internal error - wrong symbol ${s} of SMT creation fee.", ("s", dgpo.smt_creation_fee.symbol) );
   FC_ASSERT( o.smt_creation_fee.symbol == DPAY_SYMBOL || o.smt_creation_fee.symbol == BBD_SYMBOL,
   ...
   else
   {
      ...
      if( o.smt_creation_fee.symbol == DPAY_SYMBOL )
      {
         effective_elevation_fee = _db.to_bbd( o.smt_creation_fee );
      }
      else
      {
         effective_elevation_fee = _db.to_dpay( o.smt_creation_fee );
      }
   }
   ...
   _db.adjust_balance( DPAY_NULL_ACCOUNT,  o.smt_creation_fee );
...
void smt_cap_reveal_evaluator::do_apply( const smt_cap_reveal_operation& o )
{
   ...
   FC_ASSERT( smt.dpay_units_min_cap < 0 || smt.dpay_units_hard_cap < 0, "Both min cap and max hard cap have already been revealed" );

   if( smt.dpay_units_min_cap < 0 )
      try
      {
         o.cap.validate( smt.capped_generation_policy.min_dpay_units_commitment );
         _db.modify( smt, [&]( smt_token_object& smt_object )
         {
            smt_object.dpay_units_min_cap = o.cap.amount;
         });
         return;
      }
      catch( const fc::exception& e )
      {
         if( smt.dpay_units_hard_cap >= 0 )
            throw;
      }

   o.cap.validate( smt.capped_generation_policy.hard_cap_dpay_units_commitment );
   _db.modify( smt, [&]( smt_token_object& smt_object )
   {
      smt_object.dpay_units_hard_cap = o.cap.amount;
   });
}
