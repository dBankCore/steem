...
using dpay::protocol::chain_id_type;
...
{
   class variant;
   inline void to_variant( const dpay::chain::shared_string& s, variant& var )
   {
      var = fc::string( dpay::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, dpay::chain::shared_string& s )
   {
   ...
   namespace raw
   {
      ...
      template< typename T > inline void pack( dpay::chain::buffer_type& raw, const T& v )
      {
      ...
      template< typename T > inline void unpack( const dpay::chain::buffer_type& raw, T& v )
      {
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
      }

      template< typename T > inline T unpack( const dpay::chain::buffer_type& raw )
      {
         T v;
         datastream< const char* > ds( raw.data(), raw.size() );
         unpack( ds, v );
...
FC_REFLECT_ENUM( dpay::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
FC_REFLECT_TYPENAME( dpay::chain::shared_string )
FC_REFLECT_TYPENAME( dpay::chain::buffer_type )

FC_REFLECT_ENUM( dpay::chain::bandwidth_type, (post)(forum)(market) )
