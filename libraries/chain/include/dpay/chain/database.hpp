...
namespace dpay { namespace chain {
   ...
   class database : public chainbase::database
   {
      public:
          ...
          */
         void open( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t initial_supply = DPAY_INIT_SUPPLY, uint64_t shared_file_size = 0, uint32_t chainbase_flags = 0 );
          ...
          *
          * Passing slot_num == 0 returns DPAY_NULL_WITNESS
          */
         ...
         /** @return the bbd created and deposited to_account, may return BEX if there is no median feed */
         std::pair< asset, asset > create_bbd( const account_object& to_account, asset dpay, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset dpay, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& bbd, const asset& curator_bbd_value, const asset& beneficiary_value );
         ...
         /**
          * Helper method to return the current bbd value of a given amount of
          * BEX.  Return 0 BBD if there isn't a current_median_history
          */
         asset to_bbd( const asset& dpay )const;
         asset to_dpay( const asset& bbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = DPAY_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ DPAY_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ DPAY_NUM_HARDFORKS + 1 ];
