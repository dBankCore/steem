...
// but we can't keep the DPAY_RD_* constants in this header because they're used for validation and
// need to be available to dpay::protocol.
...
struct rd_system_params
{
   ...
   uint32_t        decay_per_time_unit_denom_shift = DPAY_RD_DECAY_DENOM_SHIFT;
};
...
FC_REFLECT( dpay::chain::util::rd_user_params,
   (budget_per_time_unit)
   (decay_per_time_unit)
   )

FC_REFLECT( dpay::chain::util::rd_system_params,
   (resource_unit)
