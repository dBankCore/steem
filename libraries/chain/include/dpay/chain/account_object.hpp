...
namespace dpay { namespace chain {
   ...
   class account_object : public object< account_object_type, account_object >
   {
      ...
      public:
         ...
         account_name_type reset_account = DPAY_NULL_ACCOUNT;
         ...
         bool              can_vote = true;
         uint16_t          voting_power = DPAY_100_PERCENT;   ///< current voting power of this account, it falls after every vote
          ...
          *  fields is to track the total (time * bbd_balance) that it is held. Then at the appointed time
          *  interest can be paid using the following equation:
          *
          *  interest = interest_rate * bbd_seconds / seconds_per_year
          *
          *  Every time the bbd_balance is updated the bbd_seconds is also updated. If at least
          *  DPAY_MIN_COMPOUNDING_INTERVAL_SECONDS has past since bbd_last_interest_payment then
          *  interest is added to bbd_balance.
          *
          *  @defgroup bbd_data bbd Balance Data
          */
         ///@{
         asset             bbd_balance = asset( 0, BBD_SYMBOL ); /// total bbd balance
         uint128_t         bbd_seconds; ///< total bbd * how long it has been hel
         time_point_sec    bbd_seconds_last_update; ///< the last time the bbd_seconds was updated
         time_point_sec    bbd_last_interest_payment; ///< used to pay interest at most once per month


         asset             savings_bbd_balance = asset( 0, BBD_SYMBOL ); /// total bbd balance
         uint128_t         savings_bbd_seconds; ///< total bbd * how long it has been hel
         time_point_sec    savings_bbd_seconds_last_update; ///< the last time the bbd_seconds was updated
         time_point_sec    savings_bbd_last_interest_payment; ///< used to pay interest at most once per month
         ...
         fc::array<share_type, DPAY_MAX_PROXY_RECURSION_DEPTH> proxied_vsf_votes;// = std::vector<share_type>( DPAY_MAX_PROXY_RECURSION_DEPTH, 0 ); ///< the total VFS votes proxied to this account
   ...
   typedef multi_index_container<
      ...
      indexed_by<
         ...
         >,
         ordered_unique< tag< by_dpay_balance >,
            composite_key< account_object,
         ...
         ordered_unique< tag< by_smd_balance >,
            composite_key< account_object,
               member< account_object, asset, &account_object::bbd_balance >,
               member< account_object, account_id_type, &account_object::id >
            >,
            composite_key_compare< std::greater< asset >, std::less< account_id_type > >
         >,
...
FC_REFLECT( dpay::chain::account_object,
             ...
             (reward_dpay_balance)(reward_bbd_balance)(reward_vesting_balance)(reward_vesting_dpay)
