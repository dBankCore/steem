   ...
   )
{
   ...
   if( has_hardfork( DPAY_HARDFORK_0_5__54 ) )
   {
      ...
      if( hfp.current_hardfork_version < DPAY_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == DPAY_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > DPAY_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
...
{
   DPAY_TRY_NOTIFY( pre_apply_block, block )
}
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_withdraw, VESTS_SYMBOL ), converted_dpay ) );
   }
}
...
void database::adjust_total_payout( const comment_object& cur, const asset& bbd_created, const asset& curator_bbd_value, const asset& beneficiary_value )
{
   ...
   {
      if( c.total_payout_value.symbol == bbd_created.symbol )
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( reward_tokens > 0 )
         {
            ...
            for( auto& b : comment.beneficiaries )
            {
               ...
               auto vest_created = create_vesting( get_account( b.account ), benefactor_tokens, has_hardfork( DPAY_HARDFORK_0_17__659 ) );
            ...
            auto vest_created = create_vesting( author, vesting_dpay, has_hardfork( DPAY_HARDFORK_0_17__659 ) );
            auto bbd_payout = create_bbd( author, bbd_dpay, has_hardfork( DPAY_HARDFORK_0_17__659 ) );
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( DPAY_HARDFORK_0_19__1051 ) )
            decay_rate = DPAY_RECENT_RSHARES_DECAY_RATE_HF19;
         else
            decay_rate = DPAY_RECENT_RSHARES_DECAY_RATE_HF17;
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= funds[ i ].dpay_awarded;
         });
      }
   }
}
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( from_account, old_escrow.dpay_balance );
      adjust_balance( from_account, old_escrow.bbd_balance );
...
{ try {
   ...
   const chain_id_type& chain_id = DPAY_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, DPAY_MAX_SIG_CHECK_DEPTH, is_producing() );
      }
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol )
      {
         ...
         case BBD_SYMBOL:
            ...
            assert( props.current_bbd_supply.amount.value >= 0 );
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
      }
      ...
      case DPAY_HARDFORK_0_9:
         {
            for( const std::string& acc : hardfork9::get_compromised_accounts() )
            {
               ...
               update_owner_authority( *account, authority( 1, public_key_type( "DWB8KJ4CknfhJkLkog99jCjiv9X4cCnVLWHgFYtUdWUJzw6vpwanC" ), 1 ) );

               modify( get< account_authority_object, by_account >( account->name ), [&]( account_authority_object& auth )
               {
                  auth.active  = authority( 1, public_key_type( "DWB51c4AW1tBXkzQTkwuJYtmdrUeSCwvePADcsN25cp8oZ9LMKDYW" ), 1 );
                  auth.posting = authority( 1, public_key_type( "DWB5uCWtd3e5nQ6e6wWR8DcN8CWj7UN5XmA1qGW4Rj1YaRgEbcYaG" ), 1 );
               });
            }
         }
         break;
