...
{
   set_chain_id( DPAY_CHAIN_ID_NAME );
}
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&reindex_success,&last_block_number) {
      DPAY_TRY_NOTIFY(this_->_on_reindex_done, reindex_success, last_block_number);
   } BOOST_SCOPE_EXIT_END

   try
   {
      DPAY_TRY_NOTIFY(_on_reindex_start);
...
{
   dpay_chain_id = generate_chain_id( _chain_id_name );
}
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   DPAY_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   DPAY_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   DPAY_TRY_NOTIFY( applied_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   DPAY_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   DPAY_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   DPAY_TRY_NOTIFY( on_applied_transaction, tx )
}
 ...
 * @param BEX - BEX to be converted to vesting shares
 */
asset database::create_vesting( const account_object& to_account, asset dpay, bool to_reward_balance )
{
   try
   {
       ...
       */
      asset new_vesting = dpay * ( to_reward_balance ? cprops.get_reward_vesting_share_price() : cprops.get_vesting_share_price() );
      ...
      modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_dpay += dpay;
         }
         else
         {
            props.total_vesting_fund_dpay += dpay;
   ...
   FC_CAPTURE_AND_RETHROW( (to_account.name)(dpay) )
}
...
void database::clear_null_account_balance()
{
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      auto converted_dpay = null_account.vesting_shares * gpo.get_vesting_share_price();

      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         g.total_vesting_shares -= null_account.vesting_shares;
         g.total_vesting_fund_dpay -= converted_dpay;
      });
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            if( to_deposit > 0 )
            {
               ...
               push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_dpay ) );
            }
         }
      }
      ...
      push_virtual_operation( fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_dpay ) );
   }
}
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         while( itr != cvidx.end() && itr->comment == c.id )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               auto reward = create_vesting( voter, asset( claim, DPAY_SYMBOL ), has_hardfork( DPAY_HARDFORK_0_17__659 ) );
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( reward_tokens > 0 )
         {
            ...
            for( auto& b : comment.beneficiaries )
            {
               ...
               auto vest_created = create_vesting( get_account( b.account ), asset( benefactor_tokens, DPAY_SYMBOL ), has_hardfork( DPAY_HARDFORK_0_17__659 ) );
            ...
            auto vest_created = create_vesting( author, asset( vesting_dpay, DPAY_SYMBOL ), has_hardfork( DPAY_HARDFORK_0_17__659 ) );
            ...
            push_virtual_operation( author_reward_operation( comment.author, to_string( comment.permlink ), bbd_payout.first, bbd_payout.second, vest_created ) );
            push_virtual_operation( comment_reward_operation( comment.author, to_string( comment.permlink ), to_bbd( asset( claimed_reward, DPAY_SYMBOL ) ) ) );
...
void database::process_comment_cashout()
{
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( DPAY_HARDFORK_0_17__771 ) )
      {
         ...
         funds[ fund_id ].dpay_awarded += cashout_comment_helper( ctx, *current );
      }
      else
      {
...
void database::process_funds()
{
   ...
   if( has_hardfork( DPAY_HARDFORK_0_16__551) )
   {
      ...
      const auto& producer_reward = create_vesting( get_account( cwit.owner ), asset( witness_reward, DPAY_SYMBOL ) );
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( DPAY_HARDFORK_0_14__278 ) )
               {
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      ...
      if( !median_price.is_null() && has_hardfork( DPAY_HARDFORK_0_14__230 ) )
      {
         ...
         if( percent_bbd <= DPAY_BBD_START_PERCENT )
            dgp.bbd_print_rate = DPAY_100_PERCENT;
         else if( percent_bbd >= DPAY_BBD_STOP_PERCENT )
            dgp.bbd_print_rate = 0;
         else
            dgp.bbd_print_rate = ( ( DPAY_BBD_STOP_PERCENT - percent_bbd ) * DPAY_100_PERCENT ) / ( DPAY_BBD_STOP_PERCENT - DPAY_BBD_START_PERCENT );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
void database::modify_reward_balance( const account_object& a, const asset& delta, bool check_balance )
{
   modify( a, [&]( account_object& acnt )
   {
      ...
      {
         case DPAY_ASSET_NUM_DPAY:
            acnt.reward_dpay_balance += delta;
            if( check_balance )
            {
   ...
   switch( hardfork )
   {
      case DPAY_HARDFORK_0_1:
...
#ifdef IS_TEST_NET
         {
            ...
            test_op.required_auths.insert( DPAY_INIT_MINER_NAME );
      ...
      case DPAY_HARDFORK_0_9:
         {
            for( const std::string& acc : hardfork9::get_compromised_accounts() )
            {
               ...
               update_owner_authority( *account, authority( 1, public_key_type( "DWB7sw22HqsXbz7D2CmJfmMwt9rimtk518dRzsR1f8Cgw52dQR1pR" ), 1 ) );

               modify( get< account_authority_object, by_account >( account->name ), [&]( account_authority_object& auth )
               {
                  auth.active  = authority( 1, public_key_type( "DWB7sw22HqsXbz7D2CmJfmMwt9rimtk518dRzsR1f8Cgw52dQR1pR" ), 1 );
                  auth.posting = authority( 1, public_key_type( "DWB7sw22HqsXbz7D2CmJfmMwt9rimtk518dRzsR1f8Cgw52dQR1pR" ), 1 );
               });
            }
         }
         break;
