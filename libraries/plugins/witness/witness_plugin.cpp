...
namespace detail {
   ...
   public:
      ...
      void pre_transaction( const dpay::protocol::signed_transaction& trx );
   ...
   void witness_plugin_impl::pre_transaction( const dpay::protocol::signed_transaction& trx )
   {
      ...
      if( props.total_vesting_shares.amount > 0 )
      {
         ...
         if( _db.is_producing() )
            DPAY_ASSERT( has_bandwidth,  plugin_exception,
               "Account: ${account} bandwidth limit exeeded. Please wait to transact or power up BEX.",
               ("account", a.name)
