...
{
   public:
      market_history_api( const dpay::app::api_context& ctx );

      void on_api_startup();

      /**
       * @brief Returns the market ticker for the internal BBD:BEX market
       */
      ...
      /**
       * @brief Returns the current order book for the internal BBD:BEX market.
      ...
      /**
       * @brief Returns the trade history for the internal BBD:BEX market.
      ...
      /**
       * @brief Returns the N most recent trades for the internal BBD:BEX market.
      ...
      /**
       * @brief Returns the market history for the internal BBD:BEX market.
...
FC_REFLECT( dpay::market_history::market_ticker,
   (latest)(lowest_ask)(highest_bid)(percent_change)(dpay_volume)(bbd_volume) );
FC_REFLECT( dpay::market_history::market_volume,
   (dpay_volume)(bbd_volume) );
FC_REFLECT( dpay::market_history::order,
   (price)(dpay)(bbd) );
FC_REFLECT( dpay::market_history::order_book,
   (bids)(asks) );
FC_REFLECT( dpay::market_history::market_trade,
   (date)(current_pays)(open_pays) );

FC_API( dpay::market_history::market_history_api,
   (get_ticker)
   (get_volume)
   (get_order_book)
   (get_trade_history)
