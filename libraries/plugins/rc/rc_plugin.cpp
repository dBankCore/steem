...
// 2020.748973 VESTS == 1.000 BEX when HF20 occurred on mainnet
// TODO: What should this value be for testnet?
#define DPAY_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT      2020748973

#ifndef IS_TEST_NET
#define DPAY_HF20_BLOCK_NUM                              26256743
#endif

// 1.66% is ~2 hours of regen.
// 2 / ( 24 * 5 ) = 0.01666...
#define DPAY_RC_MAX_NEGATIVE_PERCENT 166
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment )
{
   ...
   if( max_rc_creation_adjustment.symbol == DPAY_SYMBOL )
   {
   ...
   rc_plugin_skip_flags skip )
{
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( DPAY_HARDFORK_0_20 ) )
      {
         if( db.is_producing() )
         {
            DPAY_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up dpay.",
      ...
      int64_t min_mana = -1 * ( DPAY_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / DPAY_100_PERCENT;
...
void rc_plugin_impl::on_first_block()
{
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, asset( DPAY_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT, VESTS_SYMBOL ) );
   }
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == DPAY_HARDFORK_0_20 )
      {
         ...
         _db.modify( _db.get< rc_pool_object, by_id >( rc_pool_object::id_type() ), [&]( rc_pool_object& p )
         {
            ...
            {
               p.pool_array[ i ] =
                  ( ( uint128_t( params.resource_param_array[ i ].resource_dynamics_params.pool_eq ) * 90 * DPAY_1_PERCENT )
                  / DPAY_100_PERCENT ).to_uint64();
            }
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      {
         my->_enable_at_block = DPAY_HF20_BLOCK_NUM;
      }
#endif
