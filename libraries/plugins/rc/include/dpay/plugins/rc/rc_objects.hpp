...
#define DPAY_RC_DRC_FLOAT_LEVEL   (20*DPAY_1_PERCENT)
#define DPAY_RC_MAX_DRC_RATE      1000
...
enum rc_object_types
{
   ...
   rc_delegation_pool_object_type  = ( DPAY_RC_SPACE_ID << 8 ) + 3,
   rc_indel_edge_object_type       = ( DPAY_RC_SPACE_ID << 8 ) + 4,
   rc_outdel_drc_edge_object_type  = ( DPAY_RC_SPACE_ID << 8 ) + 5
};
...
class rc_delegation_pool_object : public object< rc_delegation_pool_object_type, rc_delegation_pool_object >
{
   public:
      ...
      account_name_type             account;
      dpay::chain::util::manabar   rc_pool_manabar;
};
 ...
 *
 * However, if the pool drops below DPAY_RC_DRC_FLOAT_LEVEL, DRC:RC exchange rate starts
 * to rise according to `f(x) = 1/(a+b*x)` where `x` is the pool level, and coefficients `a`,
 * `b` are set such that `f(DPAY_RC_DRC_FLOAT_LEVEL) = 1` and `f(0) = DPAY_RC_MAX_DRC_RATE`.
 *
...
class rc_outdel_drc_edge_object : public object< rc_outdel_drc_edge_object_type, rc_outdel_drc_edge_object >
{
   public:
      ...
      dpay::chain::util::manabar   drc_manabar;
      int64_t                       drc_max_mana = 0;
};

int64_t get_maximum_rc( const dpay::chain::account_object& account, const rc_account_object& rc_account );
...
FC_REFLECT( dpay::plugins::rc::rc_delegation_pool_object,
   (id)
   (account)
   (rc_pool_manabar)
   )
CHAINBASE_SET_INDEX_TYPE( dpay::plugins::rc::rc_delegation_pool_object, dpay::plugins::rc::rc_delegation_pool_index )

FC_REFLECT( dpay::plugins::rc::rc_indel_edge_object,
   (id)
   (from_account)
   (to_pool)
   (amount)
   )
CHAINBASE_SET_INDEX_TYPE( dpay::plugins::rc::rc_indel_edge_object, dpay::plugins::rc::rc_indel_edge_index )

FC_REFLECT( dpay::plugins::rc::rc_outdel_drc_edge_object,
   (id)
   (from_pool)
   (to_account)
   (drc_manabar)
...
CHAINBASE_SET_INDEX_TYPE( dpay::plugins::rc::rc_outdel_drc_edge_object, dpay::plugins::rc::rc_outdel_drc_edge_index )
