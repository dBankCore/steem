...
using fc::flat_map;
using dpay::chain::block_id_type;
...
const dpay::chain::database& chain_plugin::db() const { return my->db; }
...
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( DPAY_CHAIN_ID_NAME ), "chain ID to connect to")
#endif
         ;
}
...
void chain_plugin::plugin_startup()
{
   ...
   dpay::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef dpay::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef dpay::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const dpay::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = dpay::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const dpay::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = dpay::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
      ...
      catch( const fc::exception& e )
      {
         ...
         catch( dpay::chain::block_log_exception& )
         {
...
bool chain_plugin::accept_block( const dpay::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const dpay::chain::signed_transaction& trx )
{
   fc::optional< fc::exception > exc;
   boost::promise< bool > prom;
...
bool chain_plugin::block_is_on_preferred_chain(const dpay::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( dpay::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const dpay::chain::signed_block& block )
{
...
} } } // namespace dpay::plugis::chain::chain_apis
