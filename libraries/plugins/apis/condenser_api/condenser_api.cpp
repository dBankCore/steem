...
uint16_t api_account_object::_compute_voting_power( const database_api::api_account_object& a )
{
   ...
   // vp_t2 = DPAY_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - DPAY_100_PERCENT * (t2 - t1) / DPAY_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = DPAY_100_PERCENT * dt / DPAY_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = DPAY_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= DPAY_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to DPAY_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = DPAY_100_PERCENT;
   }
