...
void smt_create_operation::validate()const
{
   ...
   FC_ASSERT( smt_creation_fee.amount <= DPAY_MAX_SHARE_SUPPLY, "Fee must be smaller than DPAY_MAX_SHARE_SUPPLY" );
   FC_ASSERT( is_asset_type( smt_creation_fee, DPAY_SYMBOL ) || is_asset_type( smt_creation_fee, BBD_SYMBOL ), "Fee must be BEX or BBD" );
...
uint32_t smt_generation_unit::dpay_unit_sum()const
{
   uint32_t result = 0;
...
#define SMT_MIN_HARD_CAP_DPAY_UNITS     10000
#define SMT_MIN_SATURATION_DPAY_UNITS    1000
#define SMT_MIN_SOFT_CAP_DPAY_UNITS      1000
...
void smt_capped_generation_policy::validate()const
{
   ...
   FC_ASSERT( pre_soft_cap_unit.dpay_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.dpay_unit.size() <= SMT_MAX_UNIT_COUNT );
   ...
   FC_ASSERT( hard_cap_dpay_units_commitment.lower_bound >= SMT_MIN_HARD_CAP_DPAY_UNITS );
...
void smt_setup_operation::validate()const
{
   ...
   // TODO:  Support using BEX as well
   // TODO:  Move amount check to evaluator, symbol check should remain here
   FC_ASSERT( smt_creation_fee == asset( 1000000, BBD_SYMBOL ) );
}
