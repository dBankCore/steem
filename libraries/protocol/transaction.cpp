...
dpay::protocol::transaction_id_type dpay::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& dpay::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type dpay::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         DPAY_ASSERT( s.check_authority(id, enforce_membership_limit) ||
   ...
   {
      DPAY_ASSERT( s.check_authority(auth, enforce_membership_limit), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      DPAY_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id), enforce_membership_limit),
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         dpay::protocol::verify_authority(
            operations,
            result,
            get_active,
            get_owner,
