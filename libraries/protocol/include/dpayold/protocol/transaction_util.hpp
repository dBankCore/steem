#pragma once
#include <dpayold/protocol/sign_state.hpp>
#include <dpayold/protocol/exceptions.hpp>

namespace dpayold { namespace protocol {
...
void verify_authority( const vector<AuthContainerType>& auth_containers, const flat_set<public_key_type>& sigs,
                       ...
                       uint32_t max_recursion_depth = DPAYOLD_MAX_SIG_CHECK_DEPTH,
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         DPAYOLD_ASSERT( s.check_authority(id) ||
   ...
   {
      DPAYOLD_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      DPAYOLD_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
