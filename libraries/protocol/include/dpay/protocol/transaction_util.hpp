...
void verify_authority( const vector<AuthContainerType>& auth_containers, const flat_set<public_key_type>& sigs,
                       ...
                       uint32_t max_recursion_depth = DPAY_MAX_SIG_CHECK_DEPTH,
                       uint32_t max_membership = DPAY_MAX_AUTHORITY_MEMBERSHIP,
                       uint32_t max_account_auths = DPAY_MAX_SIG_CHECK_ACCOUNTS,
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         DPAY_ASSERT( s.check_authority(id) ||
   ...
   {
      DPAY_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
