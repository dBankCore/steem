...
namespace dpay { namespace protocol {
   ...
   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = DPAY_SYMBOL )
      :amount(a),symbol(id){}
...
namespace fc {
    inline void to_variant( const dpay::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  dpay::protocol::asset& vo ) { vo = dpay::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( dpay::protocol::asset, (amount)(symbol) )
FC_REFLECT( dpay::protocol::price, (base)(quote) )
