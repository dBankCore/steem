...
//
//     DPAY_DEFINE_FIXED_STRING_IMPL( 32, fc::erpair< fc::uint128_t, fc::uint128_t > )
//
...
//
//     DPAY_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, fc::uint128_t >)) )
//
#define DPAY_DEFINE_FIXED_STRING_IMPL( SIZE, STORAGE_TYPE )       \
template<>                                                         \
struct fixed_string_impl_for_size< SIZE >                          \
{                                                                  \
   typedef dpay::protocol::fixed_string_impl< STORAGE_TYPE > t;   \
};                                                                 \
                                                                   \
template<>                                                         \
...
DPAY_DEFINE_FIXED_STRING_IMPL( 16, BOOST_IDENTITY_TYPE((fc::uint128_t)) )
DPAY_DEFINE_FIXED_STRING_IMPL( 24, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, uint64_t >)) )
DPAY_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, fc::uint128_t >)) )
...
void to_variant(   const dpay::protocol::fixed_string_impl< Storage >& s, variant& v )
{
   v = std::string( s );
}

template< typename Storage >
void from_variant( const variant& v, dpay::protocol::fixed_string_impl< Storage >& s )
{
   s = v.as_string();
}

template< typename Storage >
struct get_typename< dpay::protocol::fixed_string_impl< Storage > >
{
   static const char* name()
   {
      static const std::string n =
         std::string("dpay::protocol::fixed_string<") +
         std::to_string( dpay::protocol::fixed_string_size_for_impl<Storage>::size ) +
         std::string(">");
      return n.c_str();
   }
};
