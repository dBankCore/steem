...
namespace dpay { namespace protocol {
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= DPAY_100_PERCENT, "Weight is not a BEX percentage" );
      validate_permlink( permlink );
   }
   ...
   {
      validate_account_name( from );
      FC_ASSERT( is_asset_type( amount, DPAY_SYMBOL ), "Amount must be BEX" );
      if ( to != account_name_type() ) validate_account_name( to );
      FC_ASSERT( amount > asset( 0, DPAY_SYMBOL ), "Must transfer a nonzero amount" );
   }
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( amount_to_sell, DPAY_SYMBOL ) && is_asset_type( min_to_receive, BBD_SYMBOL ) )
         ...
         "Limit order must be for the BEX:BBD market" );
