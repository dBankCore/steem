...
{
   public:
      database_api_impl( const dpay::app::api_context& ctx  );
      ~database_api_impl();
      ...
      dpay::chain::database&                _db;
      std::shared_ptr< dpay::follow::follow_api > _follow_api;
...
database_api::database_api( const dpay::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const dpay::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< dpay::follow::follow_api >( ctx );
   }
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<dpay::chain::account_member_index>();
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for dpay." );
}
...
//   if( buy_itr != end ) idump((*buy_itr));
   ...
   while(  sell_itr != end && sell_itr->sell_price.base.symbol == BBD_SYMBOL && result.bids.size() < limit )
   {
      ...
      cur.dpay = ( asset( itr->for_sale, BBD_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
      result.bids.push_back( cur );
      ++sell_itr;
   }
   while(  buy_itr != end && buy_itr->sell_price.base.symbol == DPAY_SYMBOL && result.asks.size() < limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
...
//   wdump((trx)(available_keys));
   auto result = trx.get_required_signatures( DPAY_CHAIN_ID,
                                              available_keys,
...
{
   trx.verify_authority( DPAY_CHAIN_ID,
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).active  ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).owner   ); },
                         [&]( string account_name ){ return authority( _db.get< account_authority_object, by_account >( account_name ).posting ); },
                         DPAY_MAX_SIG_CHECK_DEPTH );
                                                  ...
                                                  )const
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.back().promoted = asset(tidx_itr->promoted_balance, BBD_SYMBOL );
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   ...
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(DPAY_MAX_SHARE_SUPPLY) )  );
