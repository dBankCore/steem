...
class DebugNode( object ):
   ...
   def __init__( self, dpayd, data_dir, args='', plugins=[], apis=[], dpayd_out=None, dpayd_err=None ):
      """ Creates a dpayd debug node.
      ...
      args:
         ...
         data_dir -- The string path to an existing dpayd data directory which will be used to pull blocks from.
         args -- Other string args to pass to dpayd.
         ...
         dpayd_stdout -- A stream for dpayd's stdout. Default is to pipe to /dev/null
         dpayd_stderr -- A stream for dpayd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._dpayd_bin = None
      self._dpayd_lock = None
      self._dpayd_process = None
      self._temp_data_dir = None

      self._dpayd_bin = Path( dpayd )
      if( not self._dpayd_bin.exists() ):
         raise ValueError( 'dpayd does not exist' )
      if( not self._dpayd_bin.is_file() ):
         raise ValueError( 'dpayd is not a file' )
      ...
      if( dpayd_out != None ):
         self.dpayd_out = dpayd_out
      else:
         self.dpayd_out = self._FNULL

      if( dpayd_err != None ):
         self.dpayd_err = dpayd_err
      else:
         self.dpayd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._dpayd_lock = Lock()


   def __enter__( self ):
      self._dpayd_lock.acquire()
      ...
      dpayd = [ str( self._dpayd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      dpayd.extend( self._args )

      self._dpayd_process = Popen( dpayd, stdout=self.dpayd_out, stderr=self.dpayd_err )
      self._dpayd_process.poll()
      sleep( 5 )
      if( not self._dpayd_process.returncode ):
         self._rpc = DPayNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "dpayd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._dpayd_process != None ):
         self._dpayd_process.poll()

         if( not self._dpayd_process.returncode ):
            self._dpayd_process.send_signal( SIGINT )

            sleep( 7 )
            self._dpayd_process.poll()

            if( not self._dpayd_process.returncode ):
               self._dpayd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._dpayd_process.poll()

               if( self._dpayd_process.returncode ):
                  loggin.error( 'dpayd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._dpayd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._dpayd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key dpay debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is DPAY_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both dpayd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--dpayd', '-s', type=str, required=True, help='The location of a dpayd binary to run the debug node' )
      ...
      dpayd = Path( args.dpayd )
      if( not dpayd.exists() ):
         print( 'Error: dpayd does not exist.' )
         return

      dpayd = dpayd.resolve()
      if( not dpayd.is_file() ):
         print( 'Error: dpayd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed dpayd data directory' )
      ...
      debug_node = DebugNode( str( dpayd ), str( data_dir ), dpayd_err=sys.stderr )
