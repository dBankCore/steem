...
As you can see, the `debug_node` performs a local edit of each witness's public key:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [0,"get_witness_by_account",["jared4"]], "id": 5}' http://127.0.0.1:8990/rpc
...
If we want to take control of an account we can do so by editing its key with `debug_update_object` command like this:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [0,"lookup_account_names",[["whitehorse"]]], "id": 8}' http://127.0.0.1:8090/rpc    # find out ID of account we want is 2.2.28
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer whitehorse jared "1.234 BEX" "make -j100 money" true
    list_my_accounts
    get_account_history whitehorse -1 1000
