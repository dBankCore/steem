...
One way to think about decay is "resources expire."  BEX had relatively little activity over its
...
- The maximum RC an account can have is equal to its BEX Power (technically, VESTS).
...
of the day), and weekly variations (people use BEX more/less on weekends/holidays).  The default decay rate (347321) corresponds to a half-life of
...
100% of the BEX Power stake is constantly using all of its RC to do transactions that only require a single resource, the usage will be
...
less, than the sum of each individual user's decisions.  The budget creates market forces that influence these decisions:  You need X BEX Power
