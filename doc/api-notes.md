...
The dPay developers recommend using the first of these methods to secure the API by binding to localhost, as follows:
...
When connecting to the API, you can use the `cli_wallet` to honor a specific certificate, individual CA, or CA bundle file with the `-a` option:
    ...
    programs/cli_wallet/cli_wallet -s wss://mydomain.tld:1777 -a data/ss-cert.pem
