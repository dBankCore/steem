...
[implementation](https://github.com/dpays/dpays/blob/a0a69b10189d93a9be4da7e7fd9d5358af956e34/libraries/chain/database.cpp#L364)
of `get_account(name)`.
...
- (9a) Add `sdc_token_object_type` to `enum object_type` in `dpay_objects.hpp` and add
to `FC_REFLECT_ENUM` bubble list at the bottom of that file
- (9b) Declare (but do not define) `class sdc_token_object;` in `dpay_objects.hpp`
- (9c) Define `typedef oid< sdc_token_object > sdc_token_id_type` in `dpay_objects.hpp`
...
- (9e) All SDC objects are consensus, and therefore should exist in `dpay::chain` namespace
...
alias `typedef oid< sdc_token_object > sdc_id_type` is added to `dpay_object_types.hpp`.
