...
BOOST_AUTO_TEST_CASE( sdc_create_validate )
{
   try
   {
      ...
      op.sdc_creation_fee.amount = STEEM_MAX_SHARE_SUPPLY;
      op.validate();
...
BOOST_AUTO_TEST_CASE( setup_validate )
{
   try
   {
      ...
      (
         ...
         get_cap_commitment( SDC_MIN_HARD_CAP_STEEM_UNITS + 1 )/*hard_cap_steem_units_commitment*/,
      ...
      //FC_ASSERT( steem_unit.size() <= SDC_MAX_UNIT_ROUTES )
      ...
      //FC_ASSERT( lower_bound >= SDC_MIN_HARD_CAP_STEEM_UNITS )
      gp.min_steem_units_commitment.lower_bound = SDC_MIN_HARD_CAP_STEEM_UNITS - 1;
      op.initial_generation_policy = gp;
      STEEM_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      gp.min_steem_units_commitment.lower_bound = 1 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.min_steem_units_commitment.upper_bound = 2 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.hard_cap_steem_units_commitment.lower_bound = 1 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.hard_cap_steem_units_commitment.upper_bound = 2 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      op.initial_generation_policy = gp;
      gp.validate();
      ...
      gp.min_steem_units_commitment.lower_bound = 10 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.min_steem_units_commitment.upper_bound = 20 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.hard_cap_steem_units_commitment.lower_bound = 9 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.hard_cap_steem_units_commitment.upper_bound = 20 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      op.initial_generation_policy = gp;
      STEEM_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( min_steem_units_commitment.upper_bound <= hard_cap_steem_units_commitment.upper_bound )
      gp.hard_cap_steem_units_commitment.lower_bound = 11 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.hard_cap_steem_units_commitment.upper_bound = 19 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      op.initial_generation_policy = gp;
      STEEM_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( hard_cap_steem_units_commitment.lower_bound >= SDC_MIN_SATURATION_STEEM_UNITS * uint64_t( max_unit_ratio ) )
      gp.hard_cap_steem_units_commitment.lower_bound = 11 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.hard_cap_steem_units_commitment.upper_bound = 21 * SDC_MIN_HARD_CAP_STEEM_UNITS;
      gp.max_unit_ratio = ( ( 11 * SDC_MIN_HARD_CAP_STEEM_UNITS ) / SDC_MIN_SATURATION_STEEM_UNITS ) * 2;
      op.initial_generation_policy = gp;
      STEEM_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      //FC_ASSERT( min_soft_cap >= SDC_MIN_SOFT_CAP_STEEM_UNITS )
                                ...
                                fc::uint128_t nonce, database* db, sdc_database_fixture& dbf )
{
   ...
   FC_ASSERT( sdc_object.steem_units_min_cap < 0 && sdc_object.steem_units_hard_cap < 0 );
   ...
   FC_ASSERT( sdc_object.steem_units_min_cap < 0 && sdc_object.steem_units_hard_cap == max_cap_val );
   ...
   PUSH_OP( op, private_key );
   FC_ASSERT( sdc_object.steem_units_min_cap == min_cap_val && sdc_object.steem_units_hard_cap == max_cap_val );
...
BOOST_AUTO_TEST_CASE( sdc_cap_reveal_apply )
{
   try
   {
      ...
      setup_sdc_and_reveal_caps("alice", alice_private_key, sdcs[0], 1, SDC_MIN_HARD_CAP_STEEM_UNITS + 1, 20000, 0, db, *this);
      // Test hidden caps (1234 nonce).
      setup_sdc_and_reveal_caps("alice", alice_private_key, sdcs[1], 10000, SDC_MIN_HARD_CAP_STEEM_UNITS + 1, 20000, 1234, db, *this);
   }
   FC_LOG_AND_RETHROW()
}
