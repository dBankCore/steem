...
BOOST_FIXTURE_TEST_SUITE( sdc_tests, sdc_database_fixture )

BOOST_AUTO_TEST_CASE( sdc_create_validate )
{
   try
   {
      ACTORS( (alice) );
      ...
      op.sdc_creation_fee = ASSET( "1.000 TESTS" );
      op.symbol = get_new_sdc_symbol( 3, db );
      ...
      // Negative fee.
      op.sdc_creation_fee.amount = -op.sdc_creation_fee.amount;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      // Valid MAX_SHARE_SUPPLY
      op.sdc_creation_fee.amount = DPAY_MAX_SHARE_SUPPLY;
      op.validate();
      // Invalid MAX_SHARE_SUPPLY+1
      ++op.sdc_creation_fee.amount;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      // Invalid currency
      op.sdc_creation_fee = ASSET( "1.000000 VESTS" );
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( sdc_create_authorities )
{
   try
   {
      SDC_SYMBOL( alice, 3, db );
...
BOOST_AUTO_TEST_CASE( sdc_create_apply )
{
   try
   {
      ACTORS( (alice)(bob) )
      ...
      asset required_creation_fee = dgpo.sdc_creation_fee;
      FC_ASSERT( required_creation_fee.amount > 0, "Expected positive sdc_creation_fee." );
      ...
      // Fund with DPAY, and set fee with BBD.
      FUND( "alice", test_amount );
      // Declare fee in BBD/TBD though alice has none.
      ...
      // Declare fee in DPAY though alice has none.
      ...
      create_conflicting_sdc(op.symbol, "alice", alice_private_key);

      // Check that another user/account can't be used to create duplicating SDC even with different precision.
      create_conflicting_sdc(op.symbol, "bob", bob_private_key);

      // Check that invalid SDC can't be created
      create_invalid_sdc("alice", alice_private_key);
...
BOOST_AUTO_TEST_CASE( setup_emissions_validate )
{
   try
   {
      ...
      uint32_t an = h0lo % (SDC_MAX_NAI+1);

      FC_UNUSED(an);

      sdc_setup_emissions_operation op;
      ...
      op.control_account = "alice";
      // schedule_time <= DPAY_GENESIS_TIME;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_validate )
{
   try
   {
      ACTORS( (dany) )
      generate_block();
      asset_symbol_type dany_symbol = create_sdc("dany", dany_private_key, 3);

      sdc_set_setup_parameters_operation op;

      DPAY_REQUIRE_THROW( op.validate(), fc::exception ); // invalid symbol
      op.symbol = dany_symbol;

      op.control_account = "####";
      DPAY_REQUIRE_THROW( op.validate(), fc::exception ); // invalid account name

      op.control_account = "dany";
      op.validate();

      op.setup_parameters.emplace(sdc_param_allow_vesting());
      op.setup_parameters.emplace(sdc_param_allow_voting());
      op.validate();

      op.setup_parameters.emplace(sdc_param_allow_vesting({false}));
      op.setup_parameters.emplace(sdc_param_allow_voting({false}));
      op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( setup_emissions_apply )
{
   try
   {
      ...
      sdc_setup_emissions_operation fail_op;
      fail_op.control_account = "alice";
      fc::time_point now = fc::time_point::now();
      fail_op.schedule_time = now;
      fail_op.emissions_unit.token_unit["bob"] = 10;

      // Do invalid attempt at SDC creation.
      create_invalid_sdc("alice", alice_private_key);

      // Fail due to non-existing SDC (too early).
      ...
      auto sdcs = create_sdc_3("alice", alice_private_key);
      {
         ...
         // Do successful op with one sdc.
         sdc_setup_emissions_operation valid_op = fail_op;
         valid_op.symbol = sdc1;
         valid_op.lep_abs_amount = valid_op.rep_abs_amount = asset( 1000, valid_op.symbol );
         PUSH_OP(valid_op,alice_private_key)

         // Fail with another sdc.
         fail_op.symbol = sdc2;
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_apply )
{
   try
   {
      ...
      sdc_set_setup_parameters_operation fail_op;
      fail_op.control_account = "dany";

      // Do invalid attempt at SDC creation.
      create_invalid_sdc("dany", dany_private_key);

      // Fail due to non-existing SDC (too early).
      FAIL_WITH_OP(fail_op, dany_private_key, fc::assert_exception)

      // Create SDC(s) and continue.
      auto sdcs = create_sdc_3("dany", dany_private_key);
      {
         const auto& sdc1 = sdcs[0];
         const auto& sdc2 = sdcs[1];

         // "Reset" parameters to default value.
         sdc_set_setup_parameters_operation valid_op = fail_op;
         valid_op.symbol = sdc1;
         ...
         fail_op.setup_parameters.emplace( sdc_param_allow_vesting() );
         fail_op.setup_parameters.emplace( sdc_param_allow_voting() );
         FAIL_WITH_OP(fail_op, eddy_private_key, fc::exception);

         // Set both explicitly to false.
         valid_op.setup_parameters.clear();
         valid_op.setup_parameters.emplace( sdc_param_allow_vesting({false}) );
         valid_op.setup_parameters.emplace( sdc_param_allow_voting({false}) );
         PUSH_OP(valid_op, dany_private_key);

         // Set one to true and another one to false.
         valid_op.setup_parameters.clear();
         valid_op.setup_parameters.emplace( sdc_param_allow_vesting() );
         PUSH_OP(valid_op, dany_private_key);

         // TODO:
         // - check applying sdc_set_setup_parameters_operation after setup completed
      }
...
BOOST_AUTO_TEST_CASE( runtime_parameters_windows_validate )
{
   try
   {
      BOOST_REQUIRE( SDC_VESTING_WITHDRAW_INTERVAL_SECONDS > SDC_UPVOTE_LOCKOUT );

      ACTORS( (alice) )
      generate_block();
      asset_symbol_type alice_symbol = create_sdc("alice", alice_private_key, 3);

      sdc_set_runtime_parameters_operation op;
      ...
      op.runtime_parameters.clear();
      windows.reverse_auction_window_seconds = SDC_VESTING_WITHDRAW_INTERVAL_SECONDS;
      windows.cashout_window_seconds = windows.reverse_auction_window_seconds + 1;
      op.runtime_parameters.insert( windows );
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      windows.cashout_window_seconds = SDC_VESTING_WITHDRAW_INTERVAL_SECONDS;
      op.runtime_parameters.insert( windows );
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      op.runtime_parameters.clear();
      windows.reverse_auction_window_seconds = 0;
      windows.cashout_window_seconds = SDC_UPVOTE_LOCKOUT;
      op.runtime_parameters.insert( windows );
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      op.runtime_parameters.clear();
      windows.reverse_auction_window_seconds = 0;
      windows.cashout_window_seconds = windows.reverse_auction_window_seconds + SDC_UPVOTE_LOCKOUT + 1;
      op.runtime_parameters.insert( windows );
      op.validate();

      op.runtime_parameters.clear();
      windows.reverse_auction_window_seconds = 0;
      windows.cashout_window_seconds = SDC_VESTING_WITHDRAW_INTERVAL_SECONDS - 1;
      op.runtime_parameters.insert( windows );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( runtime_parameters_regeneration_period_validate )
{
   try
   {
      ...
      sdc_param_vote_regeneration_period_seconds_v1 regeneration;

      regeneration.vote_regeneration_period_seconds = SDC_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      op.runtime_parameters.clear();
      regeneration.vote_regeneration_period_seconds = SDC_VESTING_WITHDRAW_INTERVAL_SECONDS - 1;
      op.runtime_parameters.insert( regeneration );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( runtime_parameters_apply )
{
   try
   {
      ...
      sdc_param_vote_regeneration_period_seconds_v1 regeneration;
      regeneration.vote_regeneration_period_seconds = SDC_VESTING_WITHDRAW_INTERVAL_SECONDS / 2;
      regeneration.votes_per_regeneration_period = 1;

      sdc_param_rewards_v1 rewards;
...
BOOST_AUTO_TEST_CASE( sdc_refund_validate )
{
   try
   {
      ACTORS( (creator) )
      generate_block();
      asset_symbol_type creator_symbol = create_sdc("creator", creator_private_key, 0);

      sdc_refund_operation op;
...
BOOST_AUTO_TEST_CASE( sdc_refund_authorities )
{
   try
   {
      sdc_refund_operation op;
...
BOOST_AUTO_TEST_CASE( sdc_transfer_validate )
{
   try
   {
      ACTORS( (alice) )
...
BOOST_AUTO_TEST_CASE( sdc_transfer_apply )
{
   // This simple test touches SDC account balance objects, related functions (get/adjust)
   ...
   try
   {
      ...
      // Create SDC.
      asset_symbol_type alice_symbol = create_sdc("alice", alice_private_key, 0);
      asset_symbol_type bob_symbol = create_sdc("bob", bob_private_key, 1);

      // Give some SDC to creators.
      FUND( "alice", asset( 100, alice_symbol ) );
      FUND( "bob", asset( 110, bob_symbol ) );

      // Check pre-tranfer amounts.
      FC_ASSERT( db->get_balance( "alice", alice_symbol ).amount == 100, "SDC balance adjusting error" );
      FC_ASSERT( db->get_balance( "alice", bob_symbol ).amount == 0, "SDC balance adjusting error" );
      FC_ASSERT( db->get_balance( "bob", alice_symbol ).amount == 0, "SDC balance adjusting error" );
      FC_ASSERT( db->get_balance( "bob", bob_symbol ).amount == 110, "SDC balance adjusting error" );
      ...
      FC_ASSERT( db->get_balance( "alice", alice_symbol ).amount == 80, "SDC transfer error" );
      FC_ASSERT( db->get_balance( "alice", bob_symbol ).amount == 50, "SDC transfer error" );
      FC_ASSERT( db->get_balance( "bob", alice_symbol ).amount == 20, "SDC transfer error" );
      FC_ASSERT( db->get_balance( "bob", bob_symbol ).amount == 60, "SDC transfer error" );
...
BOOST_AUTO_TEST_CASE( comment_votable_assers_validate )
{
   try
   {
      ...
      std::array<asset_symbol_type, SDC_MAX_VOTABLE_ASSETS + 1> sdcs;
      /// Create one more than limit to test negative cases
      for(size_t i = 0; i < SDC_MAX_VOTABLE_ASSETS + 1; ++i)
      {
         asset_symbol_type sdc = create_sdc("alice", alice_private_key, 0);
         sdcs[i] = std::move(sdc);
      }
...

      {
         ...
         allowed_vote_assets ava;
         for(size_t i = 0; i < SDC_MAX_VOTABLE_ASSETS; ++i)
         {
            const auto& sdc = sdcs[i];
            ava.add_votable_asset(sdc, share_type(10 + i), (i & 2) != 0);
         }
...

      {
         ...
         {
            const auto& sdc = sdcs[i];
            ava.add_votable_asset(sdc, share_type(20 + i), (i & 2) != 0);
         }
...

      {
         ...
         BOOST_TEST_MESSAGE( "--- Testing invalid configuration of votable_assets - DPAY added to container" );
         allowed_vote_assets ava;
...
BOOST_AUTO_TEST_CASE( cap_commit_reveal_validate )
{
   try
   {
      sdc_revealed_cap reveal0;
      reveal0.amount = 0;
      reveal0.nonce = 0;
      sdc_revealed_cap revealMaxPlus;
      revealMaxPlus.amount = DPAY_MAX_SHARE_SUPPLY + 1;
      revealMaxPlus.nonce = 0;
      sdc_revealed_cap reveal1M;
      reveal1M.amount = 1000000;
      reveal1M.nonce = 0;
      sdc_revealed_cap reveal1M1234;
      reveal1M1234.amount = 1000000;
      reveal1M1234.nonce = 1234;
      sdc_revealed_cap reveal1K1234;
      reveal1K1234.amount = 1000;
      reveal1K1234.nonce = 1234;
      sdc_revealed_cap reveal1G1234;
      reveal1G1234.amount = 1000000000;
      reveal1G1234.nonce = 1234;
      sdc_revealed_cap reveal1M4321;
      reveal1M1234.amount = 1000000;
      reveal1M1234.nonce = 4321;

      // Public (i.e. non-hidden) cap
      // Test cap value too low
      sdc_cap_commitment public_cap0;
      public_cap0.lower_bound = public_cap0.upper_bound = 0;
      public_cap0.hash = fc::sha256::hash( reveal0 );
      DPAY_REQUIRE_THROW( public_cap0.validate(), fc::assert_exception );
      // Test cap value too big
      sdc_cap_commitment public_capMaxPlus;
      public_capMaxPlus.lower_bound = public_capMaxPlus.upper_bound = DPAY_MAX_SHARE_SUPPLY + 1;
      public_capMaxPlus.hash = fc::sha256::hash( revealMaxPlus );
      DPAY_REQUIRE_THROW( public_capMaxPlus.validate(), fc::assert_exception );
      // Test valid commitment cap ...
      sdc_cap_commitment public_cap1M;
      ...
      // Secret (hidden) cap
      sdc_cap_commitment hidden_cap1M1234;
      ...
      // Test reveal too low
      DPAY_REQUIRE_THROW( reveal1K1234.validate(hidden_cap1M1234), fc::assert_exception );
      // Test reveal too big
      DPAY_REQUIRE_THROW( reveal1G1234.validate(hidden_cap1M1234), fc::assert_exception );
      // Test wrong nonce
      DPAY_REQUIRE_THROW( reveal1M4321.validate(hidden_cap1M1234), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( asset_symbol_vesting_methods )
{
   try
   {
      ...
      asset_symbol_type DPay = BEX_SYMBOL;
      ...
      auto sdcs = create_sdc_3("alice", alice_private_key);
      {
         for( const asset_symbol_type& liquid_sdc : sdcs )
         {
            FC_ASSERT( liquid_sdc.is_vesting() == false );
            auto vesting_sdc = liquid_sdc.get_paired_symbol();
            FC_ASSERT( vesting_sdc != liquid_sdc );
            FC_ASSERT( vesting_sdc.is_vesting() );
            FC_ASSERT( vesting_sdc.get_paired_symbol() == liquid_sdc );
         }
      }
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( vesting_sdc_creation )
{
   try
   {
      BOOST_TEST_MESSAGE( "Test Creation of vesting SDC" );

      ACTORS((alice));
      generate_block();

      asset_symbol_type liquid_symbol = create_sdc("alice", alice_private_key, 6);
      // Use liquid symbol/NAI to confirm sdc object was created.
      auto liquid_object_by_symbol = db->find< sdc_token_object, by_symbol >( liquid_symbol );
      FC_ASSERT( ( liquid_object_by_symbol != nullptr ) );

      asset_symbol_type vesting_symbol = liquid_symbol.get_paired_symbol();
      // Use vesting symbol/NAI to confirm sdc object was created.
      auto vesting_object_by_symbol = db->find< sdc_token_object, by_symbol >( vesting_symbol );
...
BOOST_AUTO_TEST_CASE( setup_validate )
{
   try
   {
      sdc_setup_operation op;

      ACTORS( (alice) )
      generate_block();
      asset_symbol_type alice_symbol = create_sdc("alice", alice_private_key, 4);
      ...
      //FC_ASSERT( max_supply <= DPAY_MAX_SHARE_SUPPLY )
      op.max_supply = DPAY_MAX_SHARE_SUPPLY + 1;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( generation_begin_time > DPAY_GENESIS_TIME )
      op.max_supply = DPAY_MAX_SHARE_SUPPLY / 1000;
      op.generation_begin_time = DPAY_GENESIS_TIME;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      sdc_capped_generation_policy gp = get_capped_generation_policy
      (
         get_generation_unit( { { "xyz", 1 } }, { { "xyz2", 2 } } )/*pre_soft_cap_unit*/,
         get_generation_unit()/*post_soft_cap_unit*/,
         get_cap_commitment( 1 )/*min_dpay_units_commitment*/,
         get_cap_commitment( SDC_MIN_HARD_CAP_DPAY_UNITS + 1 )/*hard_cap_dpay_units_commitment*/,
         DPAY_100_PERCENT/*soft_cap_percent*/,
         1/*min_unit_ratio*/,
         2/*max_unit_ratio*/
      );
      ...
      //FC_ASSERT(decimal_places <= SDC_MAX_DECIMAL_PLACES)
      op.decimal_places = SDC_MAX_DECIMAL_PLACES + 1;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      op.decimal_places = 3;

      units to_many_units;
      for( uint32_t i = 0; i < SDC_MAX_UNIT_ROUTES + 1; ++i )
         to_many_units.emplace( "alice" + std::to_string( i ), 1 );

      //FC_ASSERT( dpay_unit.size() <= SDC_MAX_UNIT_ROUTES )
      gp.pre_soft_cap_unit.dpay_unit = to_many_units;
      gp.pre_soft_cap_unit.token_unit = { { "bob",3 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "bob2", 33 } };
      gp.pre_soft_cap_unit.token_unit = to_many_units;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //Invalid account
      gp.pre_soft_cap_unit.dpay_unit = { { "{}{}", 12 } };
      gp.pre_soft_cap_unit.token_unit = { { "xyz", 13 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "xyz2", 14 } };
      gp.pre_soft_cap_unit.token_unit = { { "{}", 15 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //Invalid account -> valid is '$from'
      gp.pre_soft_cap_unit.dpay_unit = { { "$fromx", 1 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from", 2 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "$from", 3 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from_", 4 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //Invalid account -> valid is '$from.vesting'
      gp.pre_soft_cap_unit.dpay_unit = { { "$from.vestingx", 2 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.vesting", 222 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "$from.vesting", 13 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.vesting.vesting", 3 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( dpay_unit.value > 0 );
      gp.pre_soft_cap_unit.dpay_unit = { { "$from.vesting", 0 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.vesting", 2 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "$from.vesting", 10 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from.vesting", 0 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( dpay_unit.value > 0 );
      gp.pre_soft_cap_unit.dpay_unit = { { "$from", 0 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from", 100 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "$from", 33 } };
      gp.pre_soft_cap_unit.token_unit = { { "$from", 0 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( dpay_unit.value > 0 );
      gp.pre_soft_cap_unit.dpay_unit = { { "qprst", 0 } };
      gp.pre_soft_cap_unit.token_unit = { { "qprst", 67 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "my_account2", 55 } };
      gp.pre_soft_cap_unit.token_unit = { { "my_account", 0 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.pre_soft_cap_unit.dpay_unit = { { "bob", 2 }, { "$from.vesting", 3 }, { "$from", 4 } };
      gp.pre_soft_cap_unit.token_unit = { { "alice", 5 }, { "$from", 3 } };
      op.initial_generation_policy = gp;
      op.validate();

      //FC_ASSERT( lower_bound > 0 )
      gp.min_dpay_units_commitment.lower_bound = 0;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( lower_bound >= SDC_MIN_HARD_CAP_DPAY_UNITS )
      gp.min_dpay_units_commitment.lower_bound = SDC_MIN_HARD_CAP_DPAY_UNITS - 1;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( upper_bound <= DPAY_MAX_SHARE_SUPPLY )
      gp.min_dpay_units_commitment.upper_bound = DPAY_MAX_SHARE_SUPPLY + 1;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( lower_bound <= upper_bound )
      gp.min_dpay_units_commitment.lower_bound = DPAY_MAX_SHARE_SUPPLY - 1;
      gp.min_dpay_units_commitment.upper_bound = gp.min_dpay_units_commitment.lower_bound - 1;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      gp.min_dpay_units_commitment.lower_bound = 1 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.min_dpay_units_commitment.upper_bound = 2 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.lower_bound = 1 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 2 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      op.initial_generation_policy = gp;
      gp.validate();
      ...
      //FC_ASSERT( soft_cap_percent <= DPAY_100_PERCENT )
      gp.soft_cap_percent = DPAY_100_PERCENT + 1;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( soft_cap_percent == DPAY_100_PERCENT && post_soft_cap_unit.dpay_unit.size() == 0 )
      gp.soft_cap_percent = DPAY_100_PERCENT;
      gp.post_soft_cap_unit.dpay_unit = { { "bob", 2 } };
      gp.post_soft_cap_unit.token_unit = {};
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( soft_cap_percent == DPAY_100_PERCENT && post_soft_cap_unit.token_unit.size() == 0 )
      gp.soft_cap_percent = DPAY_100_PERCENT;
      gp.post_soft_cap_unit.dpay_unit = {};
      gp.post_soft_cap_unit.token_unit = { { "alice", 3 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( soft_cap_percent != DPAY_100_PERCENT && post_soft_cap_unit.dpay_unit.size() > 0 )
      gp.soft_cap_percent = DPAY_100_PERCENT / 2;
      gp.post_soft_cap_unit.dpay_unit = {};
      gp.post_soft_cap_unit.token_unit = {};
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      //FC_ASSERT( min_dpay_units_commitment.lower_bound <= hard_cap_dpay_units_commitment.lower_bound )
      gp.min_dpay_units_commitment.lower_bound = 10 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.min_dpay_units_commitment.upper_bound = 20 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.lower_bound = 9 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 20 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( min_dpay_units_commitment.upper_bound <= hard_cap_dpay_units_commitment.upper_bound )
      gp.hard_cap_dpay_units_commitment.lower_bound = 11 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 19 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( hard_cap_dpay_units_commitment.lower_bound >= SDC_MIN_SATURATION_DPAY_UNITS * uint64_t( max_unit_ratio ) )
      gp.hard_cap_dpay_units_commitment.lower_bound = 11 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 21 * SDC_MIN_HARD_CAP_DPAY_UNITS;
      gp.max_unit_ratio = ( ( 11 * SDC_MIN_HARD_CAP_DPAY_UNITS ) / SDC_MIN_SATURATION_DPAY_UNITS ) * 2;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      sdc_capped_generation_policy gp_valid = gp;

      //FC_ASSERT( min_soft_cap >= SDC_MIN_SOFT_CAP_DPAY_UNITS )
      ...
      gp.post_soft_cap_unit.dpay_unit = { { "abc", 1 } };
      gp.post_soft_cap_unit.token_unit = { { "abc1", max_val_16 } };
      gp.pre_soft_cap_unit.token_unit = { { "abc2", max_val_16 } };
      gp.min_dpay_units_commitment.upper_bound = DPAY_MAX_SHARE_SUPPLY;
      gp.hard_cap_dpay_units_commitment.upper_bound = DPAY_MAX_SHARE_SUPPLY;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( max_dpay_accepted <= max_share_supply_u128 )
      gp.min_unit_ratio = 1;
      gp.post_soft_cap_unit.token_unit = { { "abc1", 1 } };
      gp.pre_soft_cap_unit.token_unit = { { "abc2", 1 } };
      gp.post_soft_cap_unit.dpay_unit = { { "abc3", max_val_16 } };
      gp.pre_soft_cap_unit.dpay_unit = { { "abc34", max_val_16 } };
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( setup_apply )
{
   try
   {
      ...
      asset_symbol_type bob_symbol = create_sdc( "bob", bob_private_key, 4 );
...
BOOST_AUTO_TEST_CASE( sdc_cap_reveal_validate )
{
   try
   {
   sdc_cap_reveal_operation op;
   // Set operation's fields to valid values.
   op.control_account = "alice";
   op.symbol = get_new_sdc_symbol( 4, db );
   op.validate();
   // Check invalid control account name.
   op.control_account = "@@@@@";
   DPAY_REQUIRE_THROW( op.validate(), fc::exception );
   op.control_account = "alice";
   // Check invalid SDC symbol.
   op.symbol = op.symbol.get_paired_symbol();
   DPAY_REQUIRE_THROW( op.validate(), fc::exception );
   op.symbol = op.symbol.get_paired_symbol();
   }
...
BOOST_AUTO_TEST_CASE( sdc_cap_reveal_authorities )
{
   try
   {
      sdc_cap_reveal_operation op;
...
void setup_sdc_and_reveal_caps( const account_name_type& control_account, const fc::ecc::private_key& private_key, const asset_symbol_type& sdc_symbol,
                                const share_type& min_cap_val, const share_type& max_cap_val, fc::uint128_t invalid_val,
                                fc::uint128_t nonce, database* db, sdc_database_fixture& dbf )
{
   sdc_cap_reveal_operation op;
   op.control_account = control_account;
   op.symbol = dbf.get_new_sdc_symbol( 4, db );
   // Fail due to SDC not created yet.
   FAIL_WITH_OP( op, private_key, fc::assert_exception );

   op.symbol = sdc_symbol;
   // Fail due to SDC setup phase not complete.
   FAIL_WITH_OP( op, private_key, fc::assert_exception );
   ...
   // Setup SDC
   {
      ...
      sdc_capped_generation_policy gp = dbf.get_capped_generation_policy
      (
         dbf.get_generation_unit( { { "xyz", 1 } }, { { "xyz2", 2 } } )/*pre_soft_cap_unit*/,
         dbf.get_generation_unit()/*post_soft_cap_unit*/,
         dbf.get_cap_commitment( min_cap_val, nonce )/*min_dpay_units_commitment*/,
         dbf.get_cap_commitment( max_cap_val, nonce )/*hard_cap_dpay_units_commitment*/,
         DPAY_100_PERCENT/*soft_cap_percent*/,
         1/*min_unit_ratio*/,
         2/*max_unit_ratio*/
      );
   ...
   const auto& sdc_object = db->get< sdc_token_object, by_symbol >( sdc_symbol );
   FC_ASSERT( sdc_object.dpay_units_min_cap < 0 && sdc_object.dpay_units_hard_cap < 0 );
   ...
   FC_ASSERT( sdc_object.dpay_units_min_cap < 0 && sdc_object.dpay_units_hard_cap == max_cap_val );
   ...
   PUSH_OP( op, private_key );
   FC_ASSERT( sdc_object.dpay_units_min_cap == min_cap_val && sdc_object.dpay_units_hard_cap == max_cap_val );
   // Try to reveal min cap again.
   FAIL_WITH_OP( op, private_key, fc::assert_exception );
}

BOOST_AUTO_TEST_CASE( sdc_cap_reveal_apply )
{
   try
   {
      ACTORS( (alice)(bob) )
      ...
      setup_sdc_and_reveal_caps("alice", alice_private_key, sdcs[0], 1, SDC_MIN_HARD_CAP_DPAY_UNITS + 1, 20000, 0, db, *this);
      // Test hidden caps (1234 nonce).
      setup_sdc_and_reveal_caps("alice", alice_private_key, sdcs[1], 10000, SDC_MIN_HARD_CAP_DPAY_UNITS + 1, 20000, 1234, db, *this);
   }
   FC_LOG_AND_RETHROW()
}
