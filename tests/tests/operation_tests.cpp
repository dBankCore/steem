...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( DPAY_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = DPAY_INIT_MINER_NAME;
      ...
      BOOST_REQUIRE( acct.bbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      ...
      fund( DPAY_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( DPAY_INIT_MINER_NAME, DPAY_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = DPAY_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / DPAY_BLOCK_INTERVAL );
      ...
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      sign( tx, bob_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      sign( tx, bob_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( DPAY_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / DPAY_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = dpay::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / DPAY_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( DPAY_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DPAY_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * DPAY_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( DPAY_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / DPAY_BLOCK_INTERVAL );
      ...
      generate_blocks( DPAY_CASHOUT_WINDOW_SECONDS / DPAY_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * DPAY_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - DPAY_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - DPAY_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db->head_block_time() + DPAY_MIN_VOTE_INTERVAL_SEC );

         util::manabar old_manabar = db->get_account( "alice" ).voting_manabar;
         util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), DPAY_VOTING_MANA_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - DPAY_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db->head_block_time() + fc::seconds( ( DPAY_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - DPAY_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * DPAY_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         int64_t sam_weight = old_manabar.current_mana - db->get_account( "sam" ).voting_manabar.current_mana - DPAY_VOTE_DUST_THRESHOLD;
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (DPAY_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / DPAY_VOTING_MANA_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / DPAY_100_PERCENT ).to_uint64() - DPAY_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + DPAY_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( DPAY_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / DPAY_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana - DPAY_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( DPAY_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / DPAY_100_PERCENT;
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - DPAY_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + DPAY_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - DPAY_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( DPAY_UPVOTE_LOCKOUT_SECONDS - DPAY_BLOCK_INTERVAL ) ) / DPAY_UPVOTE_LOCKOUT_SECONDS;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - DPAY_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( DPAY_UPVOTE_LOCKOUT_SECONDS - DPAY_BLOCK_INTERVAL - DPAY_BLOCK_INTERVAL ) ) / DPAY_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + DPAY_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_dpay.amount.value == vests.amount.value );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( DPAY_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + DPAY_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( DPAY_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + DPAY_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_dpay;
            gpo.total_vesting_fund_dpay = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == DPAY_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == DPAY_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == DPAY_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = DPAY_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == DPAY_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = DPAY_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( DPAY_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( DPAY_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_witness.bbd_exchange_rate == op.exchange_rate );
      BOOST_REQUIRE( alice_witness.last_bbd_exchange_update == db->head_block_time() );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- Test failure publishing with BBD base symbol" );
      ...
      // BOOST_REQUIRE( std::abs( alice_witness.bbd_exchange_rate.to_real() - op.exchange_rate.to_real() ) < 0.0000005 );
      BOOST_REQUIRE( alice_witness.last_bbd_exchange_update == db->head_block_time() );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( new_bob.bbd_balance.amount.value == ASSET( "7.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( new_alice.bbd_balance.amount.value == ASSET( "2.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- Test success converting BBD to TESTS" );
      op.owner = "bob";
      ...
      BOOST_REQUIRE( new_bob.balance.amount.value == ASSET( "3.000 TESTS" ).amount.value );
      BOOST_REQUIRE( new_bob.bbd_balance.amount.value == ASSET( "4.000 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + DPAY_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      DPAY_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "1000.000 TBD" ).amount.value );
      validate_database();
      ...
      op.expiration = db->head_block_time() + fc::seconds( DPAY_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "5.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "992.500 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "977.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.bbd_balance.amount.value == ASSET( "22.500 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "965.500 TBD" ).amount.value );
      validate_database();
      ...
      //auto start_bbd = gpo.current_bbd_supply;
      ...
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "954.500 TBD" ).amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        DPAY_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        DPAY_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        DPAY_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_authorities )
{
   try
   {
      ...
      c.expiration = db->head_block_time() + fc::seconds( DPAY_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_apply )
{
   try
   {
      ...
      create.expiration = db->head_block_time() + fc::seconds( DPAY_MAX_LIMIT_ORDER_EXPIRATION );
      tx.operations.clear();
      tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + DPAY_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + DPAY_OWNER_UPDATE_LIMIT + fc::seconds( DPAY_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + DPAY_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - DPAY_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( DPAY_OWNER_AUTH_RECOVERY_PERIOD - DPAY_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      DPAY_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      DPAY_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      DPAY_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + DPAY_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( DPAY_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      DPAY_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      DPAY_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.dpay_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when from cannot cover bbd amount" );
      signed_transaction tx;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DPAY_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      auto alice_dpay_balance = alice.balance - op.dpay_amount - op.fee;
      auto alice_bbd_balance = alice.bbd_balance - op.bbd_amount;
      auto bob_dpay_balance = bob.balance;
      auto bob_bbd_balance = bob.bbd_balance;
      auto sam_dpay_balance = sam.balance;
      auto sam_bbd_balance = sam.bbd_balance;
      ...
      BOOST_REQUIRE( escrow.bbd_balance == op.bbd_amount );
      BOOST_REQUIRE( escrow.dpay_balance == op.dpay_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_dpay_balance );
      BOOST_REQUIRE( alice.bbd_balance == alice_bbd_balance );
      BOOST_REQUIRE( bob.balance == bob_dpay_balance );
      BOOST_REQUIRE( bob.bbd_balance == bob_bbd_balance );
      BOOST_REQUIRE( sam.balance == sam_dpay_balance );
      BOOST_REQUIRE( sam.bbd_balance == sam_bbd_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.dpay_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( escrow.dpay_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DPAY_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      DPAY_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + DPAY_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.dpay_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + DPAY_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * DPAY_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.ratification_deadline == et_op.ratification_deadline );
      BOOST_REQUIRE( escrow.escrow_expiration == et_op.escrow_expiration );
      BOOST_REQUIRE( escrow.bbd_balance == et_op.bbd_amount );
      BOOST_REQUIRE( escrow.dpay_balance == et_op.dpay_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when bbd is not bbd symbol" );
      op.bbd_amount = ASSET( "1.000 TESTS" );
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.dpay_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).dpay_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).dpay_balance == ASSET( "0.800 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "0.100 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- failure when releasing more bbd than available" );
      op.dpay_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpay_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpay_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpay_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.dpay_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      DPAY_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpay_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpay_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.dpay_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is BBD" );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "10.000 TBD" ) );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring BBD to self" );
      op.amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "9.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_bbd_balance == ASSET( "1.000 TBD" ) );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success transferring BBD to other" );
      op.amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "8.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).savings_bbd_balance == ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + DPAY_SAVINGS_WITHDRAW_TIME );
      validate_database();


      BOOST_TEST_MESSAGE( "--- success withdrawing BBD to self" );
      op.amount = ASSET( "1.000 TBD" );
      op.request_id = 1;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_bbd_balance == ASSET( "9.000 TBD" ) );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing BBD to other" );
      op.amount = ASSET( "1.000 TBD" );
      op.request_id = 4;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_bbd_balance == ASSET( "8.000 TBD" ) );
      ...
      generate_blocks( db->head_block_time() + DPAY_SAVINGS_WITHDRAW_TIME - fc::seconds( DPAY_BLOCK_INTERVAL ), true );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == 4 );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).bbd_balance == ASSET( "1.000 TBD" ) );
      ...
      for( int i = 0; i < DPAY_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = DPAY_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == DPAY_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + DPAY_OWNER_AUTH_RECOVERY_PERIOD );
...
BOOST_AUTO_TEST_CASE( account_bandwidth )
{
   try
   {
      ...
      BOOST_REQUIRE( average_bandwidth == fc::raw::pack_size( tx ) * 10 * DPAY_BANDWIDTH_PRECISION );
      ...
      BOOST_REQUIRE( last_bandwidth_update == db->head_block_time() );
      BOOST_REQUIRE( average_bandwidth == total_bandwidth + fc::raw::pack_size( tx ) * 10 * DPAY_BANDWIDTH_PRECISION );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_dpay = ASSET( "0.000 TESTS" );
      op.reward_bbd = ASSET( "0.000 TBD" );
      ...
      op.reward_dpay.amount = 1000;
      op.validate();

      op.reward_dpay.amount = 0;
      op.reward_bbd.amount = 1000;
      op.validate();

      op.reward_bbd.amount = 0;
      ...
      BOOST_TEST_MESSAGE( "Testing wrong BBD symbol" );
      op.reward_dpay = ASSET( "1.000 TESTS" );
      op.reward_bbd = ASSET( "1.000 TESTS" );
      DPAY_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong VESTS symbol" );
      op.reward_bbd = ASSET( "1.000 TBD" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      db_plugin->debug_update( []( database& db )
      {
         ...
         {
            a.reward_dpay_balance = ASSET( "10.000 TESTS" );
            a.reward_bbd_balance = ASSET( "10.000 TBD" );
            a.reward_vesting_balance = ASSET( "10.000000 VESTS" );
            a.reward_vesting_dpay = ASSET( "10.000 TESTS" );
         });

         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
         {
            gpo.current_supply += ASSET( "20.000 TESTS" );
            gpo.current_bbd_supply += ASSET( "10.000 TBD" );
            gpo.virtual_supply += ASSET( "20.000 TESTS" );
            gpo.pending_rewarded_vesting_shares += ASSET( "10.000000 VESTS" );
            gpo.pending_rewarded_vesting_dpay += ASSET( "10.000 TESTS" );
         });
      });
      ...
      auto alice_dpay = db->get_account( "alice" ).balance;
      ...
      op.account = "alice";
      op.reward_dpay = ASSET( "20.000 TESTS" );
      op.reward_bbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_dpay + op.reward_dpay );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dpay_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == alice_bbd + op.reward_bbd );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_bbd_balance == ASSET( "10.000 TBD" ) );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_dpay == ASSET( "5.000 TESTS" ) );
      validate_database();
      ...
      op.reward_dpay = ASSET( "10.000 TESTS" );
      op.reward_bbd = ASSET( "10.000 TBD" );
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_dpay + op.reward_dpay );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dpay_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).bbd_balance == alice_bbd + op.reward_bbd );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).vesting_shares == alice_vests + op.reward_vests );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_dpay == ASSET( "0.000 TESTS" ) );
            validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      vest( DPAY_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "bob" ) ), DPAY_VOTING_MANA_REGENERATION_SECONDS );
      old_manabar.regenerate_mana( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - DPAY_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - DPAY_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      vest( DPAY_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DPAY_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, sam_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( gpo.delegation_return_period == DPAY_DELEGATION_RETURN_PERIOD_HF20 );
      ...
      generate_blocks( exp_obj->expiration + DPAY_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DPAY_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      DPAY_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DPAY_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), DPAY_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DPAY_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DPAY_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DPAY_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DPAY_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DPAY_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", DPAY_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DPAY_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DPAY_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( DPAY_INIT_MINER_NAME + fc::to_string( i ) ), DPAY_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), DPAY_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * DPAY_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * DPAY_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * DPAY_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      DPAY_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - DPAY_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_dpay;
            gpo.total_reward_fund_dpay = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_dpay;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_dpay.amount + db->get_account( "bob" ).reward_bbd_balance.amount + db->get_account( "sam" ).reward_vesting_dpay.amount + db->get_account( "sam" ).reward_bbd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_bbd_balance.amount + db->get_account( "alice" ).reward_vesting_dpay.amount ) == db->get_account( "bob" ).reward_vesting_dpay.amount + db->get_account( "bob" ).reward_bbd_balance.amount + 2 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_bbd_balance.amount + db->get_account( "alice" ).reward_vesting_dpay.amount ) * 2 == db->get_account( "sam" ).reward_vesting_dpay.amount + db->get_account( "sam" ).reward_bbd_balance.amount + 3 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_dpay.amount == db->get_account( "bob" ).reward_bbd_balance.amount );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_vesting_dpay.amount == db->get_account( "sam" ).reward_bbd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below DPAY_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( DPAY_MIN_BLOCK_SIZE_LIMIT - 1 );
      DPAY_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting bbd_interest_rate with negative number" );
      prop_op.props.erase( "maximum_block_size" );
      prop_op.props[ "bbd_interest_rate" ] = fc::raw::pack_to_vector( -700 );
      DPAY_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting bbd_interest_rate to DPAY_100_PERCENT + 1" );
      prop_op.props[ "bbd_interest_rate" ].clear();
      ...
      prop_op.props.erase( "bbd_exchange_rate" );
      ...
      uint64_t unit = uint64_t(1) << DPAY_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / DPAY_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / DPAY_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/DPAY_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, DPAY_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == DPAY_MIN_BLOCK_SIZE_LIMIT + 1 );

      // Setting bbd_interest_rate
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.bbd_interest_rate == 700 );
      ...
      // Setting new bbd_exchange_rate
      prop_op.props.erase( "new_signing_key" );
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "bbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET(" 1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.bbd_exchange_rate == price( ASSET( "1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
      BOOST_REQUIRE( alice_witness.last_bbd_exchange_update == db->head_block_time() );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( DPAY_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == DPAY_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << DPAY_RD_DECAY_DENOM_SHIFT ) / DPAY_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( DPAY_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( DPAY_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - DPAY_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - DPAY_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> DPAY_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( DPAY_TEMP_ACCOUNT ), [&]( account_object& a )
         {
