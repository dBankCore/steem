...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      auto vests = gpo.total_vesting_fund_dpay;
      ...
      /// because init_witness has created vesting shares and blocks have been produced, 100 BEX is worth less than 100 vesting shares due to rounding
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * DPAY_VOTE_REGENERATION_SECONDS ) / (60*60*24);
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / DPAY_100_PERCENT - DPAY_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / DPAY_100_PERCENT - DPAY_VOTE_DUST_THRESHOLD );
         ...
         BOOST_REQUIRE( db->get_account( "alice" ).voting_power == old_voting_power - ( ( old_voting_power + max_vote_denom - 1 ) * DPAY_100_PERCENT / ( 2 * max_vote_denom * DPAY_100_PERCENT ) ) );
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - db->get_account( "alice" ).voting_power ) / DPAY_100_PERCENT - DPAY_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         uint128_t new_cashout_time = db->head_block_time().sec_since_epoch() + DPAY_CASHOUT_WINDOW_SECONDS;

         BOOST_REQUIRE( new_bob.voting_power == DPAY_100_PERCENT - ( ( DPAY_100_PERCENT + max_vote_denom - 1 ) / max_vote_denom ) );
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / DPAY_100_PERCENT - DPAY_VOTE_DUST_THRESHOLD );
         ...
         new_cashout_time = db->head_block_time().sec_since_epoch() + DPAY_CASHOUT_WINDOW_SECONDS;
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( ( DPAY_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / DPAY_100_PERCENT ).to_uint64() - DPAY_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( new_sam.voting_power == DPAY_100_PERCENT - ( ( DPAY_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) );
         ...
         int64_t regenerated_power = (DPAY_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / DPAY_VOTE_REGENERATION_SECONDS;
         ...
         auto new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / DPAY_100_PERCENT ).to_uint64() - DPAY_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( DPAY_1_PERCENT * 25 * ( new_alice.voting_power ) / DPAY_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / DPAY_100_PERCENT ).to_uint64() - DPAY_VOTE_DUST_THRESHOLD;
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / DPAY_100_PERCENT - DPAY_VOTE_DUST_THRESHOLD);
      BOOST_REQUIRE( itr->rshares == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / DPAY_100_PERCENT - DPAY_VOTE_DUST_THRESHOLD );
      ...
      BOOST_REQUIRE( exp_obj->expiration == db->head_block_time() + DPAY_CASHOUT_WINDOW_SECONDS );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_dpay_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_bbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_dpay.amount + db->get_account( "sam" ).reward_vesting_dpay.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_bbd_balance.amount + db->get_account( "alice" ).reward_vesting_dpay.amount ) == db->get_account( "bob" ).reward_vesting_dpay.amount + 2 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_bbd_balance.amount + db->get_account( "alice" ).reward_vesting_dpay.amount ) * 2 == db->get_account( "sam" ).reward_vesting_dpay.amount + 3 );
   }
   FC_LOG_AND_RETHROW()
}
