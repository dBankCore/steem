...
#include <dpay/witness/witness_objects.hpp>
...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( acct.proxy == "" );
      BOOST_REQUIRE( acct.created == db.head_block_time() );
      BOOST_REQUIRE( acct.balance.amount.value == ASSET( "0.000 BEX " ).amount.value );
      BOOST_REQUIRE( acct.bbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      BOOST_REQUIRE( acct.vesting_shares.amount.value == ( op.fee * ( vest_shares / vests ) ).amount.value );
      BOOST_REQUIRE( acct.vesting_withdraw_rate.amount.value == ASSET( "0.000000 VESTS" ).amount.value );
      BOOST_REQUIRE( acct.proxied_vsf_votes_total().value == 0 );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / DPAY_100_PERCENT );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / DPAY_100_PERCENT );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - db.get_account( "alice" ).voting_power ) / DPAY_100_PERCENT );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DPAY_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / DPAY_100_PERCENT );
         ...
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( ( DPAY_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / DPAY_100_PERCENT ).to_uint64();
         ...
         auto new_rshares = ( ( fc::uint128_t( db.get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / DPAY_100_PERCENT ).to_uint64();
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / DPAY_100_PERCENT ).to_uint64();
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = asset( DPAY_MIN_ACCOUNT_CREATION_FEE + 10, DPAY_SYMBOL);
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1000.000 TESTS" ), ASSET( "1.000 TBD" ) ); // 1000 BEX : 1 BBD
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.bbd_balance.amount.value == ASSET( "100.0000 TBD" ).amount.value );
      validate_database();
...
BOOST_AUTO_TEST_CASE( account_create_with_delegation_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( delegation->min_delegation_time == db.head_block_time() + DPAY_CREATE_ACCOUNT_DELEGATION_TIME );
      auto del_amt = delegation->vesting_shares;
      auto exp_time = delegation->min_delegation_time;

      generate_block();

      BOOST_TEST_MESSAGE( "--- Test success using only BEX to reach target delegation." );

      tx.clear();
      op.fee=asset( db.get_witness_schedule_object().median_props.account_creation_fee.amount * DPAY_CREATE_ACCOUNT_WITH_DPAY_MODIFIER * DPAY_CREATE_ACCOUNT_DELEGATION_RATIO, DPAY_SYMBOL );
      ...
      fund( "alice" , asset( db.get_witness_schedule_object().median_props.account_creation_fee.amount * DPAY_CREATE_ACCOUNT_WITH_DPAY_MODIFIER * DPAY_CREATE_ACCOUNT_DELEGATION_RATIO , DPAY_SYMBOL ));
      DPAY_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / DPAY_100_PERCENT );
      BOOST_REQUIRE( itr->rshares == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / DPAY_100_PERCENT );
