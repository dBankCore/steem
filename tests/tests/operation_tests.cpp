...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( STEEM_INIT_MINER_NAME ).balance.amount + 1, BEX_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_steem.amount, BEX_SYMBOL );
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_steem_asset::from_asset( asset(STEEM_MIN_ACCOUNT_CREATION_FEE + 10, BEX_SYMBOL) );
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( STEEM_INIT_MINER_NAME, "bob", asset( 1000, BEX_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.sbd_amount.symbol = BEX_SYMBOL;
      STEEM_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when steem symbol != BEX" );
      op.sbd_amount.symbol = BBD_SYMBOL;
      op.steem_amount.symbol = BBD_SYMBOL;
      STEEM_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != BBD and fee symbol != BEX" );
      op.steem_amount.symbol = BEX_SYMBOL;
