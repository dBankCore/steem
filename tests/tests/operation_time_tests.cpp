...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      */
      ...
      BOOST_CHECK( alice_account.reward_bbd_balance == ASSET( "14288.000 TBD" ) );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      {
         alice_vshares -= ( alice_vshares * DPAY_BLOCK_INTERVAL ) / DPAY_RECENT_RSHARES_DECAY_RATE_HF19.to_seconds();
         const auto& post_rf = db.get< reward_fund_object, by_name >( DPAY_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( DPAY_INIT_MINER_NAME, accounts[i], 10000 );
      ...
      ops[0].exchange_rate = price( asset( 100000, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 105000, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ops[2].exchange_rate = price( asset(  98000, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ops[3].exchange_rate = price( asset(  97000, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ops[4].exchange_rate = price( asset(  99000, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ops[5].exchange_rate = price( asset(  97500, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 102000, DPAY_SYMBOL ), asset( 1000, BBD_SYMBOL ) );
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 99000, DPAY_SYMBOL), asset( 1000, BBD_SYMBOL ) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 99000, DPAY_SYMBOL), asset( 1000, BBD_SYMBOL ) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( ops[j].exchange_rate.base, asset( ops[j].exchange_rate.quote.amount + 10, BBD_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( bbd_interest )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).bbd_balance.amount.value == alice_bbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_bbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() ) / DPAY_SECONDS_PER_YEAR ) * gpo.bbd_interest_rate ) / DPAY_100_PERCENT ).to_uint64() );
      ...
      BOOST_REQUIRE( db.get_account( "alice" ).bbd_balance.amount.value == alice_bbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_bbd.amount.value ) * ( db.head_block_time() - start_time ).to_seconds() + alice_coindays ) / DPAY_SECONDS_PER_YEAR ) * gpo.bbd_interest_rate ) / DPAY_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( bbd_stability )
{
   try
   {
      ...
      auto db_plugin = app.register_plugin< dpay::plugin::debug_node::debug_node_plugin >();
      ...
      asset bbd_balance = asset( ( gpo.virtual_supply.amount * ( DPAY_BBD_STOP_PERCENT + 30 ) ) / DPAY_100_PERCENT, DPAY_SYMBOL ) * exchange_rate;
      ...
      BOOST_TEST_MESSAGE( "Letting percent market cap fall to 2% to verify printing of BBD turns back on" );

      // Get close to 1.5% for printing BBD to start again, but not all the way
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.bbd_balance = asset( ( 194 * bbd_balance.amount ) / 500, BBD_SYMBOL );
         });
      }, database::skip_witness_signature );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_bbd_supply = alice_bbd + asset( ( 194 * bbd_balance.amount ) / 500, BBD_SYMBOL );
         });
      }, database::skip_witness_signature );
      ...
      while( ( db.get_dynamic_global_properties().current_bbd_supply * exchange_rate ).amount >= ( db.get_dynamic_global_properties().virtual_supply.amount * DPAY_BBD_START_PERCENT ) / DPAY_100_PERCENT )
      {
         auto& gpo = db.get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( bbd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_bbd_supply, asset( ( DPAY_100_PERCENT ) * gpo.current_supply.amount ) );
