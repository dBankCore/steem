...
#if defined IS_TEST_NET && defined DPAY_ENABLE_SMT
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      op.smt_creation_fee.amount = DPAY_MAX_SHARE_SUPPLY;
      op.validate();
...
BOOST_AUTO_TEST_CASE( smt_create_apply )
{
   try
   {
      ...
      // Fund with BEX, and set fee with BBD.
      FUND( "alice", test_amount );
      // Declare fee in BBD/TBD though alice has none.
      op.smt_creation_fee = asset( test_amount, BBD_SYMBOL );
      ...
      convert( "bob", asset( too_low_fee_amount, DPAY_SYMBOL ) );
      op.smt_creation_fee = asset( too_low_fee_amount, BBD_SYMBOL );
      FAIL_WITH_OP(op, bob_private_key, fc::assert_exception);
...
BOOST_AUTO_TEST_CASE( setup_emissions_apply )
{
   try
   {
      ...
      auto smts = create_smt_3("alice", alice_private_key);
      {
         ...
         const dpay::chain::smt_token_object* smt = db->find< dpay::chain::smt_token_object, by_symbol >( fail_op.symbol );
         FC_ASSERT( smt != nullptr, "The SMT has just been created!" );
         FC_ASSERT( smt->phase < dpay::chain::smt_phase::setup_completed, "Who closed setup phase?!" );
         db->modify( *smt, [&]( dpay::chain::smt_token_object& token )
         {
            token.phase = dpay::chain::smt_phase::setup_completed;
         });
...
BOOST_AUTO_TEST_CASE( comment_votable_assers_validate )
{
   try
   {
...

      {
         ...
         BOOST_TEST_MESSAGE( "--- Testing invalid configuration of votable_assets - BEX added to container" );
         allowed_vote_assets ava;
         const auto& smt = smts.front();
         ava.add_votable_asset(smt, share_type(20), false);
         ava.add_votable_asset(DPAY_SYMBOL, share_type(20), true);
         op.extensions.insert( ava );
         DPAY_REQUIRE_THROW( op.validate(), fc::assert_exception );
      }
   }
...
BOOST_AUTO_TEST_CASE( asset_symbol_vesting_methods )
{
   try
   {
      ...
      FC_ASSERT( Vests.get_paired_symbol() == DPAY_SYMBOL );

      asset_symbol_type Bbd = BBD_SYMBOL;
      FC_ASSERT( Bbd.is_vesting() == false );
      FC_ASSERT( Bbd.get_paired_symbol() == BBD_SYMBOL );
...
BOOST_AUTO_TEST_CASE( setup_validate )
{
   try
   {
      ...
      (
         ...
         get_cap_commitment( SMT_MIN_HARD_CAP_DPAY_UNITS + 1 )/*hard_cap_dpay_units_commitment*/,
      ...
      //FC_ASSERT( dpay_unit.size() <= SMT_MAX_UNIT_ROUTES )
      ...
      //FC_ASSERT( lower_bound >= SMT_MIN_HARD_CAP_DPAY_UNITS )
      gp.min_dpay_units_commitment.lower_bound = SMT_MIN_HARD_CAP_DPAY_UNITS - 1;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      gp.min_dpay_units_commitment.lower_bound = 1 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.min_dpay_units_commitment.upper_bound = 2 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.lower_bound = 1 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 2 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      op.initial_generation_policy = gp;
      gp.validate();
      ...
      gp.min_dpay_units_commitment.lower_bound = 10 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.min_dpay_units_commitment.upper_bound = 20 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.lower_bound = 9 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 20 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( min_dpay_units_commitment.upper_bound <= hard_cap_dpay_units_commitment.upper_bound )
      gp.hard_cap_dpay_units_commitment.lower_bound = 11 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 19 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( hard_cap_dpay_units_commitment.lower_bound >= SMT_MIN_SATURATION_DPAY_UNITS * uint64_t( max_unit_ratio ) )
      gp.hard_cap_dpay_units_commitment.lower_bound = 11 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.hard_cap_dpay_units_commitment.upper_bound = 21 * SMT_MIN_HARD_CAP_DPAY_UNITS;
      gp.max_unit_ratio = ( ( 11 * SMT_MIN_HARD_CAP_DPAY_UNITS ) / SMT_MIN_SATURATION_DPAY_UNITS ) * 2;
      op.initial_generation_policy = gp;
      DPAY_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      //FC_ASSERT( min_soft_cap >= SMT_MIN_SOFT_CAP_DPAY_UNITS )
...
BOOST_AUTO_TEST_CASE( setup_apply )
{
   try
   {
      ...
      const dpay::chain::smt_token_object* smt_token = db->find< dpay::chain::smt_token_object, by_control_account >( op.control_account );
                                ...
                                fc::uint128_t nonce, database* db, smt_database_fixture& dbf )
{
   ...
   FC_ASSERT( smt_object.dpay_units_min_cap < 0 && smt_object.dpay_units_hard_cap < 0 );
   ...
   FC_ASSERT( smt_object.dpay_units_min_cap < 0 && smt_object.dpay_units_hard_cap == max_cap_val );
   ...
   PUSH_OP( op, private_key );
   FC_ASSERT( smt_object.dpay_units_min_cap == min_cap_val && smt_object.dpay_units_hard_cap == max_cap_val );
...
BOOST_AUTO_TEST_CASE( smt_cap_reveal_apply )
{
   try
   {
      ...
      setup_smt_and_reveal_caps("alice", alice_private_key, smts[0], 1, SMT_MIN_HARD_CAP_DPAY_UNITS + 1, 20000, 0, db, *this);
      // Test hidden caps (1234 nonce).
      setup_smt_and_reveal_caps("alice", alice_private_key, smts[1], 10000, SMT_MIN_HARD_CAP_DPAY_UNITS + 1, 20000, 1234, db, *this);
   }
   FC_LOG_AND_RETHROW()
}
