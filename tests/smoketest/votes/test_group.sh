...
TEST_DPAYD_PID=-1
REF_DPAYD_PID=-1
export DPAYD_NODE_PID=-1

function run_replay {
   echo Running $SCRIPT_DIR/$REPLAY_SCRIPT $TEST_DPAYD_PATH $REF_DPAYD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   $SCRIPT_DIR/$REPLAY_SCRIPT $TEST_DPAYD_PATH $REF_DPAYD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
...
open_node "tested" $TEST_DPAYD_PATH $TEST_NODE_OPT $TEST_WORK_PATH $TEST_PORT
TEST_DPAYD_PID=$DPAYD_NODE_PID

open_node "reference" $REF_DPAYD_PATH $REF_NODE_OPT $REF_WORK_PATH $REF_PORT
REF_DPAYD_PID=$DPAYD_NODE_PID

function cleanup {
   ARG=$1
   if [ $TEST_DPAYD_PID -ne -1 ]
   then
      sleep 0.5 && kill -s SIGINT $TEST_DPAYD_PID &
      wait -n $TEST_DPAYD_PID
      [ $? -ne 0 ] && echo ERROR: $TEST_DPAYD_PATH exit failed && EXIT_CODE=-1
   fi

   if [ $REF_DPAYD_PID -ne -1 ]
   then
      sleep 0.5 && kill -s SIGINT $REF_DPAYD_PID &
      wait -n $REF_DPAYD_PID
      [ $? -ne 0 ] && echo ERROR: $REF_DPAYD_PATH exit failed && EXIT_CODE=-1
   fi
...
echo TEST_DPAYD_PID: $TEST_DPAYD_PID REF_DPAYD_PID: $REF_DPAYD_PID
if [ $TEST_DPAYD_PID -ne -1 ] &&  [ $REF_DPAYD_PID -ne -1 ]; then
