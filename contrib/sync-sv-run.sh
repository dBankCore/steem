...
DPAYD_PID=`pgrep -f p2p-endpoint`
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! dpaydsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  SAVED_PID=`cat /tmp/dpaydpid`
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    gdb --batch --quiet -ex "thread apply all bt full" -ex "quit" /usr/local/dpayd-full/bin/dpayd /tmp/core.$SAVED_PID >> /tmp/stacktrace
    STACKTRACE=`cat /tmp/stacktrace`
    echo NOTIFYALERT! dpaydsync stacktrace from coredump:
  ...
  RUN_SV_PID=`pgrep -f /etc/service/dpayd`
  kill -9 $RUN_SV_PID
fi
...
BLOCKCHAIN_TIME=$(
    curl --silent --max-time 20 \
        ...
        localhost:1776 | jq -r .result.time
)
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    DPAYD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $DPAYD_PID
    echo dpaydsync: waiting for dpayd to exit cleanly

    # wait 60 seconds for dpayd to exit, to be safe.
    let WAIT_TIME=0
    while ( kill -0 $DPAYD_PID ) && [[ WAIT_TIME -le 60 ]]; do
       sleep 1
       let WAIT_TIME++
    done

    echo dpaydsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo dpaydsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! dpaydsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo dpaydsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! dpaydsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo dpaydsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! dpaydsync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo dpaydsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifydpaydsync: dpaydsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/dpayd`
    kill -9 $RUN_SV_PID
  fi
